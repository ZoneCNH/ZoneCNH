...
- CI 脚本必须有错误处理和超时设置
- 所有直接声明 runner 的 `.github/workflows/*.{yml,yaml}` job，其 `runs-on` 必须使用 `[self-hosted, Linux, X64, <approved sre/* pool>]`；历史 `ci-governance` 仅作兼容标签
- 禁止使用 `macos-*`、`windows-*` 等未批准的 hosted runner、lowercase 变体或未批准的业务/个人/模块专属额外 runner label
