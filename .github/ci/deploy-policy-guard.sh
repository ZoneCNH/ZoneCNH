...
if [[ ! -f .gitignore ]] || ! rg -q '^sre/$' .gitignore; then
  fail "本仓库必须在 .gitignore 保持 sre/；SRE 控制面不能被主页仓库收纳。"
fi

if git ls-files | rg -q '^sre/'; then
...
for workflow in "${workflows[@]}"; do
  if [[ "$workflow" =~ (deploy|deployment) ]] \
    || rg -q 'ZoneCNH/sre/\.github/workflows/deploy-contract\.yml|environment:[[:space:]]*(staging|production)|执行部署|生产部署|灰度部署|上线部署' "$workflow"; then
    ...
    if ! rg -q '^[[:space:]]*concurrency:' "$workflow"; then
      fail "$workflow 部署 workflow 必须配置 concurrency。"
    fi

    if ! rg -q 'uses:[[:space:]]*ZoneCNH/sre/\.github/workflows/deploy-contract\.yml@main' "$workflow"; then
      fail "$workflow 必须调用 ZoneCNH/sre/.github/workflows/deploy-contract.yml@main。"
    fi

    if rg -q '(^|[[:space:]|;&])(ssh|scp|rsync|kubectl|helm|systemctl)([[:space:]]|$)|docker[[:space:]]+compose' "$workflow"; then
