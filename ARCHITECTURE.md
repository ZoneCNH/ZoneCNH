...
- **规格参考实现**：binance（spec v2.2.0 Approved 分布式：natsx + 7 infra + Gin；runtime v0.2.0 bootstrap + Spot 产品线状态以 runtime 仓为准；进度 15%；runtime release blocked）
...
| 数据域                | [binance](https://github.com/ZoneCNH/binance)                   | v0.2.0 (rt) / v2.2.0 (spec) | ✅ 已有 | ░░░░ 15% | **C/S Module 规格参考实现**：spec v2.2.0 Approved（分布式 — client natsx publish / server 7 infra: natsx+redisx+pg+taosx+clickhousex+kafkax+ossx + Gin :8080）；runtime v0.2.0 bootstrap 接入 + Spot 产品线 connector/parser/mapper 部分实现；FR-009 边界 gate 文档落地；TC-020 evidence 待归档；runtime release blocked；其余 12 FR Pending（待 PR-007 runtime 实现） |
