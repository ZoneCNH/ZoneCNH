...
| 基座                  | [ossx](https://github.com/ZoneCNH/ossx)                         | v1.0.1 | ⚠️ release-only | Spec 完成 / Code 待补 | Aliyun OSS 对象存储 L2 adapter；远程仓库 0 pkg 源码（54KB，仅 docs/scripts/.repo-contract.yaml）；release v1.0.1 tag 存在但实测无 import 包；BLK-010 open（待 v1.0.2 补 pkg/ossx 源码）；非 factory；S3/MinIO/Azure/GCS Provider 仅保留扩展位 |
