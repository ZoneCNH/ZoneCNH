...
| 基座                  | [xlib-evidence](https://github.com/ZoneCNH/xlib-evidence)       | v0.1.0 | 🔨 待发布 | Spec→Code 本地验收通过，release 待补 | 证据收集与发布运行时：collect-coverage、generate-manifest、validate-manifest、report；/home/xlib-evidence 本地 go test/race/vet/coverage 89.8% 通过；v0.1.0 tag 未包含 runtime 源码，GitHub Release 待重建 |
