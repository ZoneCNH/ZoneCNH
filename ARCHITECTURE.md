...
| 基座                  | [observex](https://github.com/ZoneCNH/observex)                 | v0.3.3 | ✅ 已发布 | Spec→Code 完成 | vendor-neutral 日志、指标、追踪、健康、字段和 label policy 契约；v1.0.0 误置 tag/Release 已清理（2026-06-19） |
...
| 基座                  | [xlib-harness](https://github.com/ZoneCNH/xlib-harness)         | v0.1.6 | ✅ 已发布 | Spec→Code 完成 | 模块生成器与门禁执行器：generate/scaffold、spec-lint、boundary-check、traceability-gate、format-check；v0.1.6 发布基线已通过 go test/race/vet/coverage/benchmark/make ci；Release run 27855366871 与 main CI run 27855396013 通过；覆盖率 100.0%；pinned gitleaks CLI secret scan 已对齐 |
| 基座                  | [xlib-evidence](https://github.com/ZoneCNH/xlib-evidence)       | v0.2.4 | ✅ 已发布 | Spec→Code 完成 | 证据收集与发布运行时：collect-coverage、generate-manifest、validate-manifest、remote-evidence、report；/home/xlib-evidence 本地 go test/race/vet/build/coverage 100.0% 通过；✅ GitHub Release v0.2.4 已发布，release evidence assets 已归档 |
...
| 基座                  | [natsx](https://github.com/ZoneCNH/natsx)                       | v1.0.3 | ✅ 已发布 | Spec→Code 完成 | NATS L2 adapter：Core NATS / JetStream、Drain/reconnect/degraded health、canonical `FOUNDATIONX_NATS_*` 配置和真实 dev auth live gate 已验证；repair-slice 20/20；正式四源 98+ arbiter 与生产 TLS gate 待补（BLK-001/BLK-002）；非 factory |
...
| 基座                  | [ossx](https://github.com/ZoneCNH/ossx)                         | v1.2.0 | ✅ 已发布 | Spec→Code 完成 | Aliyun OSS L2 adapter；真实 adapters/aliyun + 流式 SPI + 完整 multipart + presign + 策略 + retry/circuit + observex hooks；pkg/ossx 100.0% 覆盖；本地实盘 integration 5/5（bucket x-go）；BLK-010 open（公开 evidence archive/external CI/downstream/soak 待补）；非 factory |
...
| **数据域 · 行情**     |                                                                 |        |           |          |                                                                                           |
| 数据域                | [market-data](https://github.com/ZoneCNH/market-data)           | v1.1.0 | ✅ 已发布 | ████ 85% | Receiver（DownstreamDispatchPort，18 测试）+ DualWriteSink（TD+Kafka 双写，6 测试）；v1.1.0 released                    |
| 数据域                | [binance](https://github.com/ZoneCNH/binance)                   | v0.2.0      | ✅ 已有 | ████ 90% | Binance Market Data C/S Module；bootstrap 接入（golden path）；domain-exchange v1.0.0 适配；build+test+gates 全通过；4 产品线 |
...
| 横切                  | [observex](https://github.com/ZoneCNH/observex)                 | v0.3.3 | ✅ 已发布 | █████ 100% | 可观测性（同时归属基座，提供底层 metrics/tracing/logging）                                |
| **独立**              |                                                                 |        |           |          |                                                                                           |
