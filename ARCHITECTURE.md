...
- **规格参考实现**：binance（spec v3.9.6 Approved 分布式：natsx + 7 infra + Gin；2026-06-28 Runtime-Anchor `/home/binance@2efc44a`；single state `23 Done / 25 Partial / 0 Drifted / 0 Pending`（48 FR total）；release_closeable=NO；P10 43 issues 全关闭；执行计划 `report/binance/production-release-execution-plan-20260628.md`）
...
| 数据域                | [binance](https://github.com/ZoneCNH/binance)                   | v0.2.0 / v3.9.6 (spec) | 🟡 本地证据 | ██░░ 48% | **C/S Module 规格参考实现**：spec v3.9.6 Approved 分布式（client natsx publish / server 7 infra + Gin :8080）；single state `23 Done / 25 Partial / 0 Drifted / 0 Pending`（48 FR total）；release_closeable=NO（Code-Done 47.9% < 90%）；Runtime-Anchor `/home/binance@2efc44a`；P10 43 issues 全关闭；执行计划已发布 |
