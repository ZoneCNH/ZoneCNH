...
- Synced `module/testkitx` docs, traceability tables, and release-state metadata to the archived verified state.

- 同步 `redisx` v1.1.0 发布投影：更新根级状态/架构/README、模块索引、验收证据与 `.foundationx` 状态快照。
- 固化模块级 Goal 文档路径为 `module/{module}/goal/goal.md`，禁止 `goal/` 目录和 `goal/1.md` 槽位。
