...
2. **同步 contracts 契约口径**：`SignalIntent` 已升入 contracts；继续补齐 P1 / P2 尚未落地的 contracts 投影（`RegimeSnapshotEvent` / `RegimeCardEvent` / `DecisionCardEvent` / `MarketRegimePort` / `MacroRegimePort` / `RegimeEnginePort`），统一下游引用
