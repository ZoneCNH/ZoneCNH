...
| [ossx](https://github.com/ZoneCNH/ossx)                   | v1.0.1      | spec/release           | non-factory     | spec=100 mat=100 tsk=100 pln=100 prm=100 cod=N/A | Aliyun OSS L2 adapter；远程仓库 0 pkg 源码（54KB，仅 docs/scripts/.repo-contract.yaml）；release v1.0.1 tag 存在但实测无 import 包；BLK-010 open；非 factory；待 v1.0.2 补 pkg/ossx 源码                |
...
> ✅ **版本 / release 注记**：公开文档是投影层；版本、release 与 factory 状态以 `.foundationx/status/index.json` + `.foundationx/blockers.json` 为准。当前 21-module projection 中 21/21 已发布 GitHub Release tag，但 ossx release 缺源码；2 open blockers（BLK-009 bootstrap, BLK-010 ossx），Foundation 整体非 factory grade。
...
| ossx                          |      ✅       |                ❌                 |                ✅                |           ❌            |           ✅           |             N/A             |                      N/A                      |   ❌    | v1.0.1; release tag 存在但仓库 0 pkg 源码; BLK-010 open（待 v1.0.2 补 pkg/ossx 源码）; 非 factory                |
...
- 组件：19 个（不含 L2.5；机器事实层另将 `domainx` 作为 L2.5 模块计入 20-module projection）；
 ...
 - 核心模块全部 21/21 GitHub Release tag 已发布，2 open blockers（BLK-009 bootstrap, BLK-010 ossx），Foundation 整体非 factory grade。
 - 存储层全部模块 GitHub Release 已发布、CI 已部署、live integration 已验证。
 - **阻塞项**：BLK-009 open（bootstrap）+ BLK-010 open（ossx 0 pkg 源码）；8 项历史 BLK 已于 2026-06-16 闭合，待 v0.1.x/v1.0.2 patch 修复。
