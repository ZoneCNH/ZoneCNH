...
| [taosx](https://github.com/ZoneCNH/taosx)                 | v1.0.1      | spec/code/release/live | factory-blocked | spec=100 mat=100 tsk=100 pln=100 prm=100 cod=100 | TDengine L2 adapter；真实 taosWS WebSocket 集成已验证；SPEC 评分 ~77（BLK-007 改善中：WHEN/THEN 已补齐，待补 Performance Budget 等）；非 factory until blocker closed                                                                                     |
...
| taosx                         |      ✅       |                ✅                 |                ✅                |           ✅            |           ✅           |             N/A             |                      N/A                      |   ❌    | v1.0.1; 8 CI workflows; 真实 taosWS 已验证; BLK-007 open（SPEC ~77，WHEN/THEN 已补齐）; 非 factory                                                                            |
...
- **阻塞项**：BLK-001 natsx 正式四源 98+ arbiter 未完成；BLK-002 natsx 生产 TLS gate 未闭合；BLK-003 clickhousex 公开 GitHub Release 未发布；BLK-006 postgresx 覆盖率 52.4% + Docker integration skip；BLK-007 taosx SPEC ~77 (WHEN/THEN 已补齐，待补 Performance Budget 等)；BLK-008 ossx API 文档 / integration evidence / quickstart / release manifest 未归档。
...
| #   | 风险                                                                                                                                                                                                   | 影响                                                              | 建议                                                     |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------- | -------------------------------------------------------- |
| R8  | natsx 正式四源 98+ arbiter 与生产 TLS gate 仍 open；postgresx 单元测试覆盖率 52.4% 且 Docker 集成测试 skip；taosx SPEC ~77 (WHEN/THEN 已补齐)；ossx API 文档 / integration evidence / quickstart / release manifest 未归档 | 阻塞对应模块 factory 投影，不阻塞上层继续按 spec/adapter 边界开发 | 按 BLK-001/002/006/007/008 关闭证据后再恢复 factory 投影 |
