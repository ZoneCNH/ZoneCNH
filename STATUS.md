...
| [natsx](https://github.com/ZoneCNH/natsx) | v1.0.1 | █████ 100% | 全管线 --force pass (spec→matrix→tasks→plan→prompt→code) | NATS L2 adapter；TLS 已实现；matrix 26 行全覆盖；claude+rules 双源 100/100；codex/copilot 待补齐 | NATS L2 adapter：Core NATS / JetStream、Drain/reconnect/degraded health、canonical FOUNDATIONX_NATS_* 配置；TLS 已实现；spec→matrix→tasks→plan 全线 --force pass；codex/copilot 待补齐 | NATS L2 adapter：Core NATS / JetStream、Drain/reconnect/degraded health、canonical `FOUNDATIONX_NATS_*` 配置；TLS 已实现；matrix 26 行全覆盖 0 孤立引用（--force pass）；tasks 6 file spec_ref/files 补齐；codex/copilot 待补齐 |
