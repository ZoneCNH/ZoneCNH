...
注：README 按域视图计数（去重 observex 后 79 仓库）；ARCH 按状态总览表（含 module/docs 独立项 81）；STATUS 按域统计表（81）。组件总数差异源于 ARCH/STATUS 计数含 module/docs 独立项而 README 不含。L2.5=4 三文档已统一（domainx 归入基座）。
