...
| [natsx](https://github.com/ZoneCNH/natsx) | v1.0.1 | █████ 100% | matrix gate pass (--force), Tasks 阶段推进中 | NATS L2 adapter：Core NATS / JetStream、Drain/reconnect/degraded health、canonical `FOUNDATIONX_NATS_*` 配置；TLS 已实现 (Config.TLS/TLSInsecure + 集成测试)；claude+rules 双源 100/100；matrix 26 行全覆盖 0 孤立引用；codex/copilot 待补齐（--force 放行） |
