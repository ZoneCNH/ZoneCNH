...
| [xlib-harness](https://github.com/ZoneCNH/xlib-harness)   | v0.1.0      | spec/code/release      | release-pending | spec=98 mat=100 tsk=98 pln=100 prm=100 cod=100   | 模块生成器与门禁执行器：generate/scaffold、spec-lint、boundary-check、traceability-gate；✅ CI 已部署；✅ GitHub Release v0.1.0 已发布                                                    |
| [xlib-evidence](https://github.com/ZoneCNH/xlib-evidence) | v0.1.0      | spec/code/release      | release-pending | spec=98 mat=100 tsk=100 pln=100 prm=100 cod=100  | 证据收集与发布运行时：collect-coverage、generate-manifest、validate-manifest、report；✅ CI 已部署；✅ GitHub Release v0.1.0 已发布                                                       |
...
| [clickhousex](https://github.com/ZoneCNH/clickhousex)     | v1.0.1      | spec/code/release/live  | release-blocked | spec=100 mat=100 tsk=100 pln=100 prm=100 cod=100 | ClickHouse；✅ GitHub Release v1.0.1 已发布；BLK-003 open；非 factory                                                                                                                             |
...
> ⚠️ **版本 / release 注记**：公开文档是投影层；版本、release 与 factory 状态以 `.foundationx/status/index.json` + `.foundationx/blockers.json` 为准。当前 20-module projection 中 20/20 已发布 GitHub Release；公开投影仍有 BLK-001/002/003/006/007/008 open，不声明 Foundation 整体 factory grade。
