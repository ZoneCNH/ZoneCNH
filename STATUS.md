...
| [natsx](https://github.com/ZoneCNH/natsx) | v1.0.1 | █████ 100% | PR #7 TLS gate + claude+rules 双源 100/100 | NATS L2 adapter：Core NATS / JetStream、Drain/reconnect/degraded health、canonical `FOUNDATIONX_NATS_*` 配置；TLS 已实现 (Config.TLS/TLSInsecure + 集成测试)；claude+rules 双源评分 100/100 (spec+matrix+tasks)；codex/copilot 外部平台待运行 |
...
- **阻塞项**：natsx codex/copilot 外部平台评分待运行（claude+rules 双源已通过 100/100，TLS gate 已完成 PR #7）；clickhousex 已发布 v1.0.1；xlibgate CLI 已实现（PR #23）；testkitx code 阶段已完成（PR #13）；postgresx foundationx 依赖已移除（PR #8 → PR #9）
...
| # | 风险 | 影响 | 建议 |
| -- | ---- | ---- | ---- |
| R8 | natsx codex/copilot 外部平台评分待运行（claude+rules 双源 100/100）；postgresx 单元测试覆盖率 52.4% 加 Docker 集成测试 skip 中 | 不阻塞上层开发 | 按需推进剩余补证 |
