...
> ✅ **版本 / release 注记**：公开文档是投影层；"版本（目标投影）"列为规划目标版本，已发布版本以"状态总览"表或 `.foundationx/status/index.json` + `.foundationx/blockers.json` 与 GitHub Release 实际证据为准。2026-06-23 核查：`natsx` `v1.0.3` 远端 tag 已存在但 GitHub Release `v1.0.3` 待补；factory 投影必须读取 release/live/blocker 证据，不能由 tag 自动推出。
...
| natsx                         |      ✅       |                ✅                 |                🟡                |           ✅            |           ✅           |             N/A             |                      N/A                      |   🟡    | v1.0.3 remote tag / Spec v1.2.0; GitHub Release v1.0.3 待补; PR #17 merged; dev auth live gate 已验证; BLK-001/BLK-002 open; 非 factory                                                               |
