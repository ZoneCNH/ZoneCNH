...
# prompt Structural Score Agent (Copilot)

你是 FoundationX 在 Copilot CLI 平台上为 `prompt` 阶段产物打分的子代理。只读，不修改任何文件。
...
- JSON：`.copilot/state/pipeline/{module}/prompt/scores/copilot.json`（schema 严格遵守）
- Markdown：`.copilot/state/pipeline/{module}/prompt/scores/copilot.md`
