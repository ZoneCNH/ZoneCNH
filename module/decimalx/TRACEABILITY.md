...
|-------|-------------|----------|----------|--------------|--------|
| FR-DEC-001 | decimal-immutable | AC-DEC-001 | TC-DEC-001, TC-DEC-007 | `go test -race ./...` | ✅ |
| FR-DEC-002 | parse-grammar | AC-DEC-002 | TC-DEC-002 | `go test -run TestParse -fuzz=FuzzParse` | ✅ |
| FR-DEC-003 | string-output | AC-DEC-003 | TC-DEC-003 | `go test -run TestString` | ✅ |
| FR-DEC-004 | arithmetic-exact | AC-DEC-004 | TC-DEC-005 | `go test -run TestArithmetic` | ✅ |
| FR-DEC-005 | arithmetic-rounding | AC-DEC-004 | TC-DEC-005 | `go test -run TestQuoScale` | ✅ |
| FR-DEC-006 | rounding-modes | AC-DEC-009 | TC-DEC-008 | `go test -run TestRounding` | ✅ |
| FR-DEC-007 | json-encoding | AC-DEC-005 | TC-DEC-003 | `go test -run TestJSON -fuzz=FuzzJSON` | ✅ |
| FR-DEC-008 | sql-scan | AC-DEC-006 | TC-DEC-004 | `go test -run TestSQLScan` | ✅ |
| FR-DEC-009 | money-currency | AC-DEC-007 | TC-DEC-006 | `go test -run TestMoney` | ✅ |
| FR-DEC-010 | error-identity | AC-DEC-008 | TC-DEC-008 | `go test -run TestErrors` | ✅ |
...
|-------|------|----------|--------------|--------|
| BR-DEC-001 | `Decimal` 不可变——所有 arithmetic 返回新值，不修改 receiver | TC-DEC-001, TC-DEC-007 | TC-DEC-001 (`Coeff()` 返回 copy), TC-DEC-007 (并发读取无 data race) | ✅ |
| BR-DEC-002 | `Parse` 默认使用 `DefaultLimits`，拒绝超出 precision/scale 上限的输入 | TC-DEC-002 | TC-DEC-002 (Parse 拒绝空串/指数/空白/非法 scale) + boundary fuzz tests | ✅ |
| BR-DEC-003 | 非精确除法必须显式 rounding，零值 Context 不得静默截断 | TC-DEC-005 | TC-DEC-005 (`QuoExact(1,3)` 返回 ErrNonTerminating) | ✅ |
| BR-DEC-004 | JSON 序列化仅允许带引号十进制字符串，禁止 JSON number | TC-DEC-003 | TC-DEC-003 (JSON round-trip `"1.23000"` → Decimal → `"1.23000"`) | ✅ |
| BR-DEC-005 | SQL `Scan(float64)` 必须失败——防止静默精度损失 | TC-DEC-004 | TC-DEC-004 (`Scan(float64)` 返回 ErrFloatScanRejected) | ✅ |
| BR-DEC-006 | `Money` 跨币种运算必须失败——防止隐式汇率假设 | TC-DEC-006 | TC-DEC-006 (`Money(USD).Add(Money(EUR))` 返回 ErrCurrencyMismatch) | ✅ |
