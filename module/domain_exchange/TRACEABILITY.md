...
|-------|------|----------|--------------|--------|
| BR-EXC-001 | Exchange 不支持某 capability 时返回 typed error，不得 panic 或静默空值 | TC-EXC-002 | TC-EXC-002 验证 ErrUnsupportedCapability + `GOWORK=off go test -race ./...` | ✅ |
| BR-EXC-002 | prod/paper 下单必须有 ClientID，保证幂等 | TC-EXC-001 | TC-EXC-001 验证 Validate() 拒绝空 ClientID | ✅ |
| BR-EXC-003 | adapter 必须把交易所原始错误映射为 typed ExchangeError，不暴露原始 DTO | TC-EXC-003 | TC-EXC-003 验证 errors.Is/As 可识别 wrapped error | ✅ |
...
| BR-EXC-006 | Registry 重复注册返回错误，不允许覆盖 | TC-EXC-004 | TC-EXC-004 验证并发注册行为 + `GOWORK=off go test -race ./...` | ✅ |
| BR-EXC-007 | WS channel 关闭规则：ctx cancel 后 channel 可预测关闭；不允许 goroutine leak | TC-EXC-006 | TC-EXC-006 验证 leak test + `GOWORK=off go test -race ./...` | ✅ |
