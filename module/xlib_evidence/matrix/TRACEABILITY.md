...
| FR-005 | report: 聚合多模块证据→生成跨模块统一报告 | AC-005 | TC-005 | `go test -run 'Test(ReportsRenderValidatedManifest|StoreAppendEnforcesHashChain|StoreReturnsImmutableManifests)'` | ✅ |
...
| BR-002 | 覆盖率低于100.0%不得发布 | TC-001 | 覆盖率边界测试(99.99%拒绝, 100.00%通过) | ✅ |
...
|-------|-------------|---------|
| TC-001 | FR-001 | `go test -run 'Test(ParseGoCoverageProfile|CoverageValidateRejectsThresholdAndPercentTamper|NewCoverageNormalizesTime|MeetsThresholdBoundary)'` — 覆盖率解析、100.0%边界和时间归一化 |
...
|-------|-----------|-----------|-------------|--------|
| AC-001 | FR-001 / BR-002 | 覆盖率数据被结构化收集; 覆盖率<100.0%拒绝发布 | `go test -run 'Test(ParseGoCoverageProfile|CoverageValidateRejectsThresholdAndPercentTamper|NewCoverageNormalizesTime|MeetsThresholdBoundary)'` | ✅ |
