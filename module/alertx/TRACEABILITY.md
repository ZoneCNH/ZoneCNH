...
| ----- | ---- | -------- | ---- | ------------ | ------ |
| BR-001 | 告警不丢失：评估产出的 AlertEvent 必须进入去重/通知流程，不得静默丢弃 | TC-020 | TASK-004 | CI gate `alert-no-loss-check` 阻塞；递增 `foundationx_alertx_alerts_dropped` counter | ⏳ |
| BR-002 | 通知幂等：同一 DedupKey + event.ID 的通知最多发送一次 | TC-020 | TASK-005 | CI gate `notify-idempotent-check` 阻塞 | ⏳ |
| BR-003 | SuppressWindow 强制：规则未设时用全局默认（非零），零窗口规则被拒绝 | TC-004 | TASK-003 | 返回 `ErrSuppressWindowZero`，启动阻塞 | ⏳ |
| BR-004 | 规则 DSL 校验失败必须阻塞启动 | TC-002 | TASK-002 | 返回 `ErrRuleInvalid`，退出码非零 | ⏳ |
| BR-005 | 通知渠道配置必须完整：规则引用的 channel ID 必须已定义 | TC-006 | TASK-005 | 返回 `ErrChannelUnknown`，启动阻塞 | ⏳ |
| BR-006 | severity 映射不可降级：critical 必须尝试 paging 渠道 | TC-005 | TASK-004 | 降级行为记 error，CI gate 阻塞 | ⏳ |
| BR-007 | 自观测指标命名符合 `foundationx_alertx_<measure>` | TC-018 | TASK-007 | CI gate `metrics-contract-check` 阻塞 | ⏳ |
...
| ------ | -------- | ----------- | ---- | ------------ | ------ |
| NFR-001 | 性能 | 规则评估（单事件，100 条规则）< 1ms | TASK-002 | Benchmark | ⏳ |
| NFR-002 | 性能 | 告警端到端（评估->通知，单 webhook）< 100ms | TASK-005 | 集成测试计时 | ⏳ |
| NFR-003 | 性能 | 通知重试退避指数 1s/2s/4s | TASK-005 | 单元测试 | ⏳ |
| NFR-004 | 性能 | 内存占用（10k 活跃告警）< 100MB | TASK-007 | Soak | ⏳ |
| NFR-005 | 质量 | 单元测试覆盖率 >= 80% | TASK-007 | `go tool cover` | ⏳ |
| NFR-006 | 安全 | race 检测通过（零 data race） | TASK-007 | `go test -race` | ⏳ |
| NFR-007 | 质量 | vet 检查通过（零警告） | TASK-007 | `go vet` | ⏳ |
| NFR-008 | 质量 | lint 检查通过（零错误） | TASK-007 | `golangci-lint` | ⏳ |
| NFR-009 | 安全 | Secret 扫描通过（零命中） | TASK-007 | `gitleaks` | ⏳ |
| NFR-010 | 架构 | 无反向依赖（observex/contracts/kernel 不依赖 alertx） | TASK-007 | boundary check | ⏳ |
...
| TC-011 | FR-004 | `go test ./... -run TestNotifier_ChannelFailure` |
| TC-015 | FR-007 | `go test ./... -run TestRuleReload_ConcurrentWithEval` |
...
> 说明：全部 FR/BR/NFR/AC/TC 当前为 ⏳（已定义待实现）。Task 总数 = TASK-002~008 共 7 项。S8 Code 阶段实现后，Status 从 ⏳ 翻转为 ✅。
...
| 日期 | 变更 | 来源 |
| ---- | ---- | ---- |
| 2026-06-29 | Goal 管线对齐：§2 BR 表新增 Task 列；§3 NFR 表新增 Task 列；§6 覆盖率仪表盘标准化为 Done/覆盖率格式 | ZoneCNH |
