...
|-------|------|----------|--------------|--------|
| BR-001 | 订单必须先通过 riskx 才能提交交易所 | TC-ORD-002 | TC-ORD-002 riskx 调用断言 | ✅ | |
| BR-002 | 终态订单不可再修改（FILLED/CANCELLED/REJECTED/EXPIRED） | TC-ORD-001 | TC-ORD-001 终态转换拒绝断言 | ✅ | |
| BR-003 | SOR 父订单状态 = 所有子订单状态的聚合 | TC-ORD-004 | TC-ORD-004 父订单聚合状态断言 | ✅ | |
| BR-004 | 撤单操作幂等（对已终态的订单撤单返回成功） | TC-ORD-005 | TC-ORD-005 重复撤幂等断言 | ✅ | |
| BR-005 | 订单 ID 全局唯一 | TC-ORD-009 | TC-ORD-009 orderID 唯一性断言 | ✅ | |
