...
| binance | [#434](https://github.com/xhyperium/binance/pull/434) | 升级 v0.5.1 + `boolToInt32` DRY 收敛 |
| binance | [#435](https://github.com/xhyperium/binance/pull/435) | consumer JSON payload 字段名 + `Code`→`RejectCode` + BNC 码→canonical |
| binance | [#436](https://github.com/xhyperium/binance/pull/436) | `live_assembly_test.go` config 字段漂移修复 |
| binance | [#437](https://github.com/xhyperium/binance/pull/437) | `depth_test.go` RejectCode 类型不匹配 + `http_ingest_endpoint_test` canonical 语义 + go.sum tidy |
| binance | [#438](https://github.com/xhyperium/binance/pull/438) | 升级 v0.5.2 + 移除 4 处 `IngestReject.RequestID` 冗余写入 |
