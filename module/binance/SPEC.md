...
**WHEN** 模块内声明 generic/cross-exchange storage/query/strategy 所有权
...
- 允许：server → `module/domain_market` 语义类型、`module/natsx`、`module/redisx`、`module/taosx`、`module/postgresx`、`module/clickhousex`、`module/kafkax`、`module/ossx`、Gin、shared config/observability
...
### natsx JetStream Interface (v2.1.1)

```go
// MarketFactEnvelope is published by client through natsx JetStream.
// natsx JetStream subject 格式（v2.1.1，替代 gRPC MarketDataService）
...
| TC-022 | FR-009 | CI | go.mod 依赖合规检查 | natsx/redisx/kafkax/postgresx/taosx/clickhousex/ossx/Gin direct 依赖状态符合 BOUNDARY-GATES |
...
| Gate | 命令 | 通过条件 |
|------|------|----------|
| No legacy binance-market | `/home/binance/scripts/boundary-gates.sh` + `BOUNDARY-GATES.md` §2 | 零 forbidden 引用 |
| Client must not import server | `/home/binance/scripts/boundary-gates.sh` + `BOUNDARY-GATES.md` §3 | 零 client→server import |
| Server must not import client | `/home/binance/scripts/boundary-gates.sh` + `BOUNDARY-GATES.md` §4 | 零 server→client import |
| No cs runtime dependency | `/home/binance/scripts/boundary-gates.sh` + `BOUNDARY-GATES.md` §5 | 零 runtime `internal/cs` import |
| No same-process C/S | `/home/binance/scripts/boundary-gates.sh` + `BOUNDARY-GATES.md` §6 | client/server 只通过 natsx 网络通信 |
| Binance-specific storage ownership | `/home/binance/scripts/boundary-gates.sh` + `BOUNDARY-GATES.md` §7 | 只拥有 Binance-specific storage/query/API/fanout，不声明 generic/cross-exchange ownership |
| Wire contract externality | `/home/binance/scripts/boundary-gates.sh` + `BOUNDARY-GATES.md` §8 | 零 local proto/gRPC ingest schema |
| Domain-market source | `/home/binance/scripts/boundary-gates.sh` + `BOUNDARY-GATES.md` §9 | 零独立 canonical enum 定义 |
| Admin boundary | `/home/binance/scripts/boundary-gates.sh` + `BOUNDARY-GATES.md` §10 | 零跨模块 admin mutation |
| go.mod dependency compliance | `/home/binance/scripts/boundary-gates.sh` + `BOUNDARY-GATES.md` §11 | natsx/redisx/kafkax/postgresx/taosx/clickhousex/ossx/gin 均为 direct |
...
- [ ] Boundary gates 可在 CI 执行（FR-009, `/home/binance/scripts/boundary-gates.sh` + BOUNDARY-GATES.md）
- [ ] Runtime mapping 未将 generic/cross-exchange market_data/query/strategy ownership 放在 Binance 内（BR-006）
...
| G0-3 | `redisx`/`taosx`/`postgresx`/`clickhousex`/`ossx`/`kafkax`/Gin ownership chain ready | server SPEC §7/§9 + v2.1.1 RUNTIME-MAPPING.md | ✅ |
...
| G0-6 | BOUNDARY-GATES.md 全部 10 门禁有 runtime 脚本 | `/home/binance/scripts/boundary-gates.sh` 已对齐；runtime 修复前必须 FAIL | PENDING |

> **文档契约链已闭合；运行时未完成** — 本 SPEC 可作为 v2.1.1 实现基线，但不得声明 Release Done。必须先让 `/home/binance/scripts/boundary-gates.sh`、build/test/race/vet/lint 全部 PASS 后才能更新实现状态。
