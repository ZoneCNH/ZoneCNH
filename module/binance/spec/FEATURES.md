...
| BR-008 Wire Contract Externality | Done | `/home/workspace/binance/BOUNDARY-GATES.md` §8 证明无本地 `.proto`/gRPC ingest schema；runtime 使用 natsx subject + `contracts` canonical 契约（v0.5.0，ADR-007 闭环），binance 经 `internal/ingestcodec` boundary 序列化，canonical 语义外置。 |
