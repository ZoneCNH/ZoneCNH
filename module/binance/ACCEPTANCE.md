...
| 层级 | 含义 | 当前口径 |
| --- | --- | --- |
| L1 | 文档与追溯矩阵一致性证据。 | 本文件、`FEATURES.md`、`TRACEABILITY.md`、`IMPLEMENTATION-PLAN.md` 与 root `SPEC.md` v2.2.3 对齐。 |
...
| 验收面 | 命令 | 通过条件 |
| --- | --- | --- |
| 文档补丁格式 | `cd /home/ZoneCNH/.worktree/workspaces/fix/binance-issues-20260623 && git diff --check -- module/binance` | 无 trailing whitespace 或 patch 格式错误。 |
| 追溯锚点覆盖 | `cd /home/ZoneCNH/.worktree/workspaces/fix/binance-issues-20260623 && rg -n "FR-001|FR-010|TC-001|TC-022|AC-001|AC-035" module/binance/SPEC.md module/binance/TRACEABILITY.md module/binance/FEATURES.md module/binance/ACCEPTANCE.md` | 根级 FR、AC、TC 锚点在规格、追溯和补齐文档中可定位。 |
| Boundary gates | `cd /home/binance && bash scripts/boundary-gates.sh` | Runtime SHA `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3` 上 `boundary-gates 10/10 PASS`。 |
| Runtime tests | `cd /home/binance && go test ./...` | Runtime SHA `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3` 上 `go test ./... PASS`。 |
| Smoke self-test | `cd /home/binance && XGO_BINANCE_SMOKE_SELF_TEST=1 go run ./cmd/binance-smoke` | Runtime SHA `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3` 上 `XGO_BINANCE_SMOKE_SELF_TEST=1 go run ./cmd/binance-smoke PASS`。 |
...
| AC-009 | FR-003 | Publish 等待 PubAck，失败返回可观测错误；C/S 可在不同机器独立启动且 boundary gate 验证无跨进程 import。 | TC-005 | Pending |
...
| AC-025 | FR-006d | 归档路径遵循 `binance/{product_line}/{symbol}/{YYYY}/{MM}/{DD}/{event_type}.parquet`；每日定时查询 cutoff（now - 90d）之前的 taosx 数据。 | TC-016 | Pending |
| AC-026 | FR-006d | 删除 `taosx` 旧分区前必须校验对象 ETag；ossx ETag 验证通过后才执行 taosx.Delete（先写冷再删热）。 | TC-016 | Pending |
| AC-027 | FR-006d | ETag 不匹配时停止删除并报警；ossx path 与 parquet object 格式可回放校验。 | TC-017 | Pending |
| AC-028 | FR-008 | Server 通过 `kafkax` 发送 `binance.{product_line}.{event_type}.v1`；kafkax topic = versioned Kafka topic。 | TC-018 | Pending |
...
| AC-031 | FR-009 | Boundary gate 禁止 `binance-client` 导入 server internals；server 源码无 `internal/client` 或 `internal/cs` 导入。 | TC-020 | L1/L2 PASS |
| AC-032 | FR-009 | Boundary gate 禁止 `binance-server` 导入 client internals；任何代码 reintroduce `binance-market` 引用时 no-legacy gate 失败。 | TC-021 | L1/L2 PASS |
| AC-033 | FR-009 | Boundary gate 禁止 `binance-market` 与 runtime `internal/cs` 回流；go.mod 中 natsx/redisx/postgresx/taosx/clickhousex/kafkax/ossx/gin 均保持 direct 依赖。 | TC-022 | L1/L2 PASS |
| AC-034 | FR-009 | Boundary gate 禁止 Binance 模块定义 canonical market domain 或本地 proto/gRPC ingest schema；BOUNDARY-GATES §5（cs 包禁止）+ §6（同进程禁止）+ §10（go.mod 合规）全 PASS。 | TC-020, TC-022 | L1/L2 PASS |
| AC-035 | FR-009 | `BOUNDARY-GATES` 全量检查通过，且 client/server 边界、进程边界、依赖边界与 schema 约束均保持可审计。 | TC-020, TC-022 | L1/L2 PASS |
...
| TC-005 | FR-003, BR-002, BR-003 | Boundary gate（跨进程边界检查） | L1/L2 PASS | Runtime SHA `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS`。 |
...
| TC-020 | FR-009, BR-005 | Boundary gate（cs 包/client 包 import 检查） | L1/L2 PASS | Runtime SHA `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS`。 |
| TC-021 | FR-009, BR-001 | Boundary gate（no-legacy 引用检查） | L1/L2 PASS | Runtime SHA `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS`。 |
| TC-022 | FR-009, BR-009 | Boundary gate（go.mod 合规） | L1/L2 PASS | Runtime SHA `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS`。 |
...
| FR-006d | AC-025~AC-027 | TC-016~TC-017 | Not Closed |
| FR-008 | AC-028~AC-030 | TC-018~TC-019 | Not Closed |
| FR-009 | AC-031~AC-035 | TC-020~TC-022 | L1/L2 Boundary Closed; L3 release not closed |
...
| 所有 FR closed | Not Done | FR-001~FR-009 状态全部闭合。 |
| 所有 AC passed | Not Done | AC-001~AC-035 全部有测试证据。 |
| 所有 TC passed | Not Done | TC-001~TC-022 全部 PASS。 |
| Runtime L1/L2 evidence | L1/L2 PASS | Runtime SHA `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS`、`go test ./... PASS`、`XGO_BINANCE_SMOKE_SELF_TEST=1 go run ./cmd/binance-smoke PASS`。 |
| Coverage and performance evidence | Not Done | 覆盖率、延迟、吞吐、重放与故障注入报告归档。 |
| L3 live / production / GitHub CI / release evidence | Blocked / Not Claimed | live Binance、production credentials、GitHub CI、release 证据未取得，不作为当前完成声明。 |

## 6. 当前验收缺口

| 缺口 | 风险 | 下一步关闭动作 |
| --- | --- | --- |
| L3 release 证据缺失 | 不能从 L1/L2 boundary evidence 推断 live 或 release complete。 | 后续由主线程补齐 live Binance、production credentials、GitHub CI、release 证据；未取得前保持 Blocked/Pending。 |
...
| FR-003~FR-008 Pending；FR-009 L1/L2 only | C/S runtime、存储、API、广播、归档未闭合；Boundary enforcement 只有 L1/L2 本地证据。 | 按 `IMPLEMENTATION-PLAN.md` 和 tasks 顺序实现，补充 L3 release 证据并更新 traceability。 |
| TC-021/TC-022 L3 Pending | Boundary enforcement 已有 L1/L2 本地证据，但无 GitHub CI 证据。 | 后续接入 CI 并记录独立 run 链接。 |
