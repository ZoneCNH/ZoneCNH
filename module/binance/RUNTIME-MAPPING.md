...
- [COMPUTED, HIGH] 未证明范围：live Binance websocket、独立 client/server 进程端到端、`NakWithDelay(5s)`、dead-letter/parking/DLQ、`redisx/postgresx/taosx/ossx` 持久化、`kafkax` fanout、`/api/v1` market/query API、release tag 与 release evidence。
...
| `cmd/binance-server` | 服务端：消费 natsx → 处理存储 → 提供 API | API :8080, admin :8082 | natsx, redisx, postgresx, taosx, kafkax, ossx, gin | Partial：PR #11 已证明 HTTP `/ingest` receiver boundary；2026-06-24 gated ManualAck/MaxDeliver immediate Nak 子集通过；尚无独立进程、`NakWithDelay`、dead-letter、存储、fanout、query 证据 |
