...
> 状态：**已完成** — 含代码实态审计（§12），可作为 SPEC v2.0.0 升级输入
...
> 审计日期：2026-06-21
> 审计方式：直接读取 `/home/binance/` 实际源码
> 标签：[COMPUTED] HIGH

本节记录 `github.com/ZoneCNH/binance` **当前实际代码状态**与目标架构（§2）之间的精确差距。

### 12.1 通信层 — 非 gRPC，非 natsx

```
当前实现（实态）:
  internal/cs/types.go        ← in-process 共享类型包
  internal/cs/doc.go
  internal/client/sender.go   ← IngestClient 接口（Go interface 直调）
  internal/server/ingest.go   ← IngestServer.Process()（同进程）

  运行模式: client 与 server 在同一进程通过 Go interface 直接调用
  文档描述: "gRPC bidi stream" — 与实际不符（骨架文档与实现脱节）
  目标: natsx JetStream（跨进程，持久化，at-least-once）
```

| 层 | 文档描述 | 实际代码 | 差距 |
|---|---|---|---|
| C/S 通信 | gRPC bidi stream (contracts) | in-process Go interface (cs 包) | 两者均非目标 natsx |
| Client 输出 | 到 gRPC server | 调用 IngestClient.Ingest() | 目标是 natsx.Publish() |
| Server 输入 | gRPC stream recv | cs.IngestRequest 参数 | 目标是 natsx.Subscribe() |

### 12.2 Client 端 — 需删除组件

```
当前存在（需删除）:
  internal/client/spool.go        ← SQLite in-memory spool
  internal/client/checkpoint.go   ← ACK-driven checkpoint
  internal/client/sender.go       ← Sender{Ingest, Spool, Checkpoint}
  internal/client/idempotency.go  ← key 生成（保留逻辑，移入 publisher）

当前仅 Spot 实现:
  internal/client/spot.go         ← gorilla/websocket，仅 Spot

需新增:
  internal/client/publisher/      ← natsx JetStream 发布器
  internal/client/connectors/
    usdm.go                       ← USDⓈ-M connector（缺失）
    coinm.go                      ← COIN-M connector（缺失）
    options.go                    ← Options connector（缺失）
```
...
```
当前占位符（需重写）:
  internal/server/idempotency.go  ← memoryIdempotencyStore（非 redisx）
  internal/server/dispatch.go     ← RecordingSink（in-memory，非 kafkax）

全量缺失（需新增）:
  internal/server/consumer/       ← natsx 消费（核心入口）
  internal/server/processor/      ← 事件处理管线
  internal/server/storage/
    taos_writer.go                ← 时序写入
    pg_catalog.go                 ← 合约元数据
    pg_idempotency_log.go         ← 幂等持久备份
    oss_archiver.go               ← 历史归档
  internal/server/cache/          ← redisx 热缓存
  internal/server/api/            ← Gin REST API（market_data 消费）
```

### 12.4 go.mod 依赖差距

```
当前状态（/home/binance/go.mod）:

缺失（未声明）:
  github.com/gin-gonic/gin        ← Web API 框架 — 完全缺失
  github.com/ZoneCNH/ossx         ← 历史归档 — 完全缺失

Indirect（需升为 direct）:
  github.com/ZoneCNH/natsx   v1.0.0  // indirect  → 需升为 direct
  github.com/ZoneCNH/redisx  v1.0.1  // indirect  → 需升为 direct
  github.com/ZoneCNH/kafkax  v1.0.2  // indirect  → 需升为 direct
  github.com/ZoneCNH/postgresx v1.0.0 // indirect → 需升为 direct
  github.com/ZoneCNH/taosx  v1.0.1  // indirect   → 需升为 direct

已正确声明（direct）:
  github.com/ZoneCNH/bootstrap    v0.1.0
  github.com/ZoneCNH/domain_market v1.1.0
  github.com/ZoneCNH/domain_exchange v1.0.0
  github.com/ZoneCNH/domainx      v1.0.1
...
+   github.com/ZoneCNH/ossx v1.0.0
    ...
  )

  require (
-   github.com/ZoneCNH/natsx v1.0.0 // indirect
-   github.com/ZoneCNH/redisx v1.0.1 // indirect
-   github.com/ZoneCNH/kafkax v1.0.2 // indirect
-   github.com/ZoneCNH/postgresx v1.0.0 // indirect
-   github.com/ZoneCNH/taosx v1.0.1 // indirect
    ...
  )
```
...
```
当前存在:
  cmd/binance-server/  ← 已有（main.go 在 binance-server/ 目录）
  cmd/binance-smoke/   ← 冒烟测试（使用同进程 IngestServer）

缺失（需新增）:
  cmd/binance-client/  ← 独立采集器进程（需创建）
```

**注意**：当前 `binance-server` 目录实际在根目录（`/home/binance/binance-server/main.go`），
应迁移到 `cmd/binance-server/main.go` 以匹配标准 Go 项目布局。

### 12.6 内部 cs 包的角色

```
当前: internal/cs/types.go 定义 IngestRequest/IngestResult/IngestAck 等类型
      作为 client 和 server 的共享类型桥

v2.0.0 目标: natsx wire format（JSON 序列化的 domain_market 类型）
             cs 包可作为历史文档保留，但不再是运行时通信契约

迁移路径:
  1. cs.IngestRequest → json.Marshal(domainmarket.MarketFactEnvelope)
  2. cs.IngestResult  → natsx ACK/NAK 机制
  3. cs.IngestAck     → 删除（JetStream 内建 ACK）
```
...
| 优先级 | 变更 | 影响 | 难度 |
|:------:|------|------|:----:|
| P0 | 新增 `cmd/binance-client/main.go` | 启用独立采集进程 | 低 |
| P0 | 新增 `internal/client/publisher/` (natsx) | C/S 通信层替换 | 中 |
| P0 | 新增 `internal/server/consumer/` (natsx) | 服务端入口 | 中 |
| P1 | 重写 `internal/server/idempotency/` (redisx) | 幂等去重可靠性 | 中 |
| P1 | 新增 `internal/server/storage/taos_writer.go` | 时序存储 | 中 |
| P1 | 新增 `internal/server/cache/` (redisx) | 热缓存 + API 性能 | 低 |
| P2 | 新增 `internal/server/api/` (Gin REST) | market_data 消费接口 | 中 |
| P2 | 新增 `internal/server/storage/pg_catalog.go` | 合约元数据持久化 | 中 |
| P2 | 重写 `internal/server/dispatch/` (kafkax) | 跨域事件发布 | 中 |
| P3 | 新增 `internal/server/storage/oss_archiver.go` | 历史归档 | 中 |
| P3 | 补全 connectors: usdm/coinm/options | 4 产品线覆盖 | 高 |
| P3 | go.mod: 添加 gin + ossx，升级 indirect→direct | 依赖声明规范化 | 低 |
| P4 | 删除 spool.go / checkpoint.go / sender.go | Client 简化 | 低 |
| P4 | 迁移/删除 internal/cs 包 | 通信契约清理 | 中 |
