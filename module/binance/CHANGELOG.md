...
- Spec-Reference: `module/binance/spec/SPEC.md` v3.9.7
- 治理规则：`module/binance/gate/RULES.md` R9 文档存在性

---

## [v3.9.7] — 2026-07-02 基于 DATA-INTEGRITY-E2E-20260701.md v3.9 对抗性自审全面降级 release_closeable

### Summary

基于 `report/binance/DATA-INTEGRITY-E2E-20260701.md` v3.9（20 轮 200 维度对抗性自审）识别 58 个运行时缺口（GAP-E1~E58），将 SPEC 从 v3.9.6 的"48 Done / release_closeable=YES"全面降级为"12 Done / 23 Partial / 13 Drifted / release_closeable=NO"。本变更闭合"issue 已 close ≠ 运行时缺口已修复"的治理漏洞（GAP-E58）。

### Changed — `module/binance/spec/SPEC.md` (v3.9.6 → v3.9.7)

- **frontmatter**：`Spec-Version` v3.9.6 → v3.9.7；`Current-State` 从 `48 Done / 0 Partial / 0 Drifted / 0 Pending` 改为 `12 Done / 23 Partial / 13 Drifted / 0 Pending`；`release_closeable` YES → **NO**；`Open-P10-Issues` 从 0 改为 58
- **§1 Goal**：声明 v3.9.7 是规格与运行时事实对齐版本，引用 CONSTITUTION §75/§166/§509/§7/§8/§9/§10.4（修复 GAP-E51）
- **§5 State Model**：状态重映射说明
- **§7 Functional Requirements**：48 FR 按 RUNTIME-GAP-MATRIX 重新标注 Done/Partial/Drifted，每行补 Runtime-Gap 列
- **§8 Business Requirements**：修复 BR 编号跳号问题（GAP-E53，原 BR 编号缺 BR-008）
- **§21 Release Gate**：PRG-001~007 状态表全面重核，PRG-003/004/005/006/007 降级
- **§22 Change History**：追加 v3.9.7 条目
- **§23 Stop Condition**：从"已达到"改为"未达到"
- **Appendix A**（新增）：CONSTITUTION 章节引用清单
- **Appendix B**（新增）：运行时缺口摘要

### Changed — `module/binance/matrix/TRACEABILITY.md`

- **frontmatter**：Matrix-Version v3.9.6 → v3.9.7；Current-State 与 SPEC 对齐
- **§2 FR Matrix**：新增 `Runtime-Gap` 列，每 FR 关联 GAP-E 编号（CLAUDE.md §5.1）
- **§3 AC**：AC-007 标为 Drifted（issue 关闭≠运行时修复）
- **§4 PRG**：PRG-003/004/005/006/007 实际状态降级
- **§6 Summary**：仪表盘从 §2 派生

### Added — `module/binance/RUNTIME-GAP-MATRIX.md`（v1.0 新建）

- 58 个 GAP-E × FR × AC × 严重度 × 工时 五列映射表
- §1 P0/P1 缺口（13 个，~32 人天）
- §2 P2 缺口（22 个，~28.5 人天）
- §3 P3 缺口（19 个，~13 人天）
- §4 汇总统计
- §5 修复路径建议（阶段 A/B/C）
- §6 GitHub Issue 反向追溯（待补）
- §7 文件溯源

### Fixed

- GAP-E51：SPEC 无引用 CONSTITUTION 章节号 → SPEC Appendix A 新增
- GAP-E52：CHANGELOG v3.9.7 比 SPEC 提前一版 → SPEC 同步升至 v3.9.7
- GAP-E53：BR 编号跳号（缺 BR-008）→ SPEC §8 修复
- GAP-E57：evidence 完全无 GAP-E 引用 → RUNTIME-GAP-MATRIX.md §7 溯源

### 自审范围

20 轮 200 维度对抗性自审（v3.9 用户裁决"源码 + spec 双向 20 轮"）：
- R1~R7：v3.8 已完成（report v3.8 §11 GAP-E1~E36）
- R8：OWASP 安全 10 维度 → GAP-E37
- R9：性能/资源 10 维度 → GAP-E38
- R10：可观测性 10 维度 → GAP-E39
- R11：可测试性 10 维度 → GAP-E40
- R12：依赖/版本/供应链 10 维度 → 合规
- R13：部署/运维 10 维度 → GAP-E41~E43
- R14：兼容性/演进 10 维度 → 合规
- R15：治理/法律 10 维度 → GAP-E44/E45
- R16：数据治理 10 维度 → 合规
- R17：并发/数据竞争 10 维度 → 合规
- R18~R21：spec 文档结构/FR 状态漂移/边界裁决/AC 验证 → GAP-E46~E51
- R22：DR/BRE 漂移 → GAP-E52
- R23：跨制品一致性 → GAP-E53
- R24：gate 文件完整性 → GAP-E54/E55
- R25：design/ADR 一致性 → GAP-E56
- R26：evidence 完整性 → GAP-E57
- R27：todo/issues 投影 → GAP-E58

### 用户裁决记录

- 对齐策略：**全面降级 release_closeable + 全量重写**（用户 2026-07-02 裁决）
- 自审范围：**源码 + spec 双向 20 轮（最重）**
- 补齐内容：新建 RUNTIME-GAP-MATRIX.md + 更新 SPEC.md Spec-Version + 更新 TRACEABILITY.md + 更新 CHANGELOG.md（四项全选）
