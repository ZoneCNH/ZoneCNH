...
| PR-001 | Root docs v2.1.1 | `SPEC.md`、`TRACEABILITY.md`、`BOUNDARY-GATES.md`、`ACCEPTANCE.md`、`IMPLEMENTATION-PLAN.md` 版本和追溯一致。 |
...
2. 调整 `go.mod` direct deps：`natsx/redisx/kafkax/postgresx/taosx/clickhousex/ossx/gin` 必须为 direct。
3. 删除或隔离 `internal/cs`、spool、checkpoint、sender 等同进程 C/S 路径。
4. client 实现：catalog -> parser -> connector -> normalizer -> mapper -> idempotency -> publisher -> admin。
5. server 实现：consumer -> processor -> redisx idempotency/cache/lock -> taosx -> postgresx -> clickhousex -> ossx -> kafkax -> Gin API -> admin。
6. 补齐边界测试、集成测试、失败注入、build/test/race/vet/lint/secret scan。
7. 只有 runtime evidence 全部通过后，才允许把 `TRACEABILITY.md` 的 Pending 改为 Implemented。
