...
- Runtime-Evidence-Repo: `/home/binance`
- Runtime-Evidence-SHA: `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`
- Evidence-Levels: L1 = 文档/追溯一致性；L2 = `/home/binance` 本地命令证据；L3 = live Binance / production credentials / GitHub CI / release evidence。
- L2 Command Evidence: `boundary-gates 10/10 PASS`; `go test ./... PASS`; `XGO_BINANCE_SMOKE_SELF_TEST=1 go run ./cmd/binance-smoke PASS`。
- 本矩阵仅把边界治理项标为 L1/L2 PASS；未声明 L3 live、production、CI、release 完成。
...
| FR-008 | kafkax Broadcast：将 accepted facts 广播到 versioned Kafka topic `binance.{product_line}.{event_type}.v1` | AC-029 ~ AC-031 | TC-018, TC-019 | SERVER-014 | ⬜ Pending |
| FR-009 | Boundary Enforcement：本地 gate 阻断 client/server 跨界、cs 包引用、go.mod 合规 | AC-032 ~ AC-035 | TC-020 ~ TC-022 | SERVER-008 | L1/L2 PASS — `/home/binance` @ `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS`；`go test ./... PASS`；`XGO_BINANCE_SMOKE_SELF_TEST=1 go run ./cmd/binance-smoke PASS`；L3 live/production/CI/release 未声明 |
...
|-------|----------|----------|------|----------|
| BR-001 | No binance-market：禁止在 active architecture 中引用 `binance-market` | Boundary Gate: BOUNDARY-GATES.md §2 | TASK-BINANCE-ROOT-000 | L1/L2 PASS — `/home/binance` @ `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS` |
| BR-002 | Client Must Not Import Server Internals | Boundary Gate: BOUNDARY-GATES.md §3 | CLIENT-014, SERVER-010 | L1/L2 PASS — `/home/binance` @ `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS` |
| BR-003 | Server Must Not Import Client Internals | Boundary Gate: BOUNDARY-GATES.md §4 | SERVER-010 | L1/L2 PASS — `/home/binance` @ `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS` |
...
| BR-005 | No cs Package：禁止 `internal/cs` 包；禁止 C/S 同进程运行 | Boundary Gate: BOUNDARY-GATES.md §5, §6 | SERVER-008 | L1/L2 PASS — `/home/binance` @ `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS` |
| BR-006 | Server Owns Binance Storage：market_data 禁止直连 binance 的 taosx/postgresx/redisx/ossx | Boundary Gate: BOUNDARY-GATES.md §7 | SERVER-012 ~ SERVER-016 | L1/L2 PASS — `/home/binance` @ `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS` |
| BR-007 | No Domain Ownership：模块不得定义 canonical domain semantics SSOT，必须引用 `domain_market` | Boundary Gate: BOUNDARY-GATES.md §9 | TASK-BINANCE-ROOT-004 | L1/L2 PASS — `/home/binance` @ `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS` |
| BR-008 | Wire Contract Externality：不得定义自己的 proto，接口协议通过 natsx subject + JSON envelope | Boundary Gate: BOUNDARY-GATES.md §8 | SERVER-010, CLIENT-014 | L1/L2 PASS — `/home/binance` @ `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS` |
| BR-009 | go.mod Dependency Compliance：natsx/redisx/postgresx/taosx/clickhousex/kafkax/ossx/gin 必须保持 direct 依赖 | Boundary Gate: BOUNDARY-GATES.md §10 | SERVER-015, SERVER-016 | L1/L2 PASS — `/home/binance` @ `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；`boundary-gates 10/10 PASS` |
...
| NFR-019 | API Key / Secret 从环境变量读取，不硬编码 | §19 Security | 安全审查与 fixture 检查（Pending；无 L2 完成证据） |
...
| TC-005 | FR-003 | BR-002, BR-003 | Boundary gate（跨进程边界检查） | L1/L2 PASS |
...
| TC-020 | FR-009 | BR-005 | Boundary gate（cs 包/client 包 import 检查） | L1/L2 PASS |
| TC-021 | FR-009 | BR-001 | Boundary gate（no-legacy 引用检查） | L1/L2 PASS |
| TC-022 | FR-009 | BR-009 | Boundary gate（go.mod 7 infra + gin + clickhousex direct） | L1/L2 PASS |
...
| AC-010 | FR-003 | C/S 可在不同机器独立启动，共用外部 NATS JetStream 连接地址；Boundary gate 验证无跨进程 import | TC-005 |
...
| AC-032 | FR-009 | server 源码无 `internal/client` 或 `internal/cs` 导入（Boundary gate） | TC-020 |
| AC-033 | FR-009 | 任何代码 reintroduce `binance-market` 引用时 Boundary no-legacy gate 失败 | TC-021 |
| AC-034 | FR-009 | go.mod 中 natsx/redisx/postgresx/taosx/clickhousex/kafkax/ossx/gin 均保持 direct 依赖 | TC-022 |
| AC-035 | FR-009 | BOUNDARY-GATES §5（cs 包禁止）+ §6（同进程禁止）+ §10（go.mod 合规）全 PASS | TC-020 |
...
| 业务规则 (BR) | 9 | 9 | 100% | BR-001 ~ BR-009 全部有 Boundary Gate 或 TC |
...
| BR→验证覆盖率 | — | 9/9 | 100% | — |
| AC→验证覆盖率 | — | 47/47 | 100% | — |
| L1/L2 边界证据 | — | FR-009 + BR-001/002/003/005/006/007/008/009 + TC-005/020/021/022 | 已回填 | 仅代表本地边界/测试/smoke 证据；L3 release 仍未闭合 |
| Release / L3 状态 | — | 0/15 FR | 0% | live/production/GitHub CI/release 未声明 |
...
| 2026-06-23 | v2.2.3 | **L1/L2 runtime evidence 语义回填**：统一 `/home/binance` evidence SHA `f30322e00794f9f0af7353c4f8e1cd2b6cc398b3`；仅记录 `boundary-gates 10/10 PASS`、`go test ./... PASS`、`XGO_BINANCE_SMOKE_SELF_TEST=1 go run ./cmd/binance-smoke PASS` 三项本地命令证据；FR-008 Kafka topic 收敛为 `binance.{product_line}.{event_type}.v1`；FR-009、BR-001/002/003/005/006/007/008/009、TC-005/020/021/022 改为 L1/L2 PASS；BR-004 与 L3 release/live/production/CI 仍保持未闭合 | ZoneCNH |
