...
- Matrix-Version: v2.1.1
- Last-Updated: 2026-06-21
- Spec-Reference: `module/binance/SPEC.md` v2.1.1

---

## §1 FR 追溯表

> **v2.1.1 变更摘要**：FR-006 拆分为 6a(taosx)/6b(postgresx)/6c(redisx cache)/6d(ossx)；FR-007 扩展 analytics API(7a)；新增 FR-010（clickhousex OLAP 存储）、FR-011（分布式协调锁）；subject 命名统一 `um_perp`/`cm_perp`；Error 码扩展至 BNC-013；Performance Budget 扩展至 20 项；FR-009 回退为 runtime gate pending，禁止无脚本证据计为完成。
...
| FR-009 | Boundary Enforcement：CI gate 阻断 client/server 跨界、cs 包引用、go.mod 合规 | AC-032 ~ AC-035 | TC-020 ~ TC-022 | SERVER-008 | ⬜ Pending — BOUNDARY-GATES.md v2.1.1 与 runtime 脚本已对齐；需 `/home/binance/scripts/boundary-gates.sh` 全 PASS 后关闭 |
...
| BR-005 | No cs Package：禁止 `internal/cs` 包；禁止 C/S 同进程运行 | CI Gate: BOUNDARY-GATES.md §5, §6 | SERVER-008 | Pending — BOUNDARY-GATES v2.1.1 与 runtime 脚本对齐；等待脚本 PASS |
| BR-006 | Server Owns Binance-Specific Storage：`binance/server` 只拥有 Binance-specific taosx/postgresx/redisx/clickhousex/ossx/kafkax/Gin，不拥有 generic/cross-exchange market_data 平台 | CI Gate: BOUNDARY-GATES.md §7 | SERVER-012 ~ SERVER-017 | Pending |
| BR-007 | No Domain Ownership：模块不得定义 canonical domain semantics SSOT，必须引用 `domain_market` | CI Gate: BOUNDARY-GATES.md §9 | TASK-BINANCE-ROOT-004 | Pending |
| BR-008 | Wire Contract Externality：不得定义自己的 proto，接口协议通过 natsx subject + JSON envelope | CI Gate: BOUNDARY-GATES.md §8 | SERVER-010, CLIENT-014 | Pending |
| BR-009 | go.mod Dependency Compliance：natsx/redisx/kafkax/postgresx/taosx/clickhousex/ossx/gin 均为 direct | CI Gate: BOUNDARY-GATES.md §11 | SERVER-015, SERVER-016, SERVER-017 | Pending |
...
| TC-020 | FR-009 | BR-005 | CI gate（cs 包/client 包 import 检查） | Pending（`/home/binance/scripts/boundary-gates.sh` 全 PASS 后关闭） |
...
| AC-026 | FR-006d | 每日定时查询 cutoff（now - 90d）之前的 taosx 数据 | TC-016 |
| AC-027 | FR-006d | ossx ETag 验证通过后才执行 taosx.Delete（先写冷再删热） | TC-016 |
| AC-028 | FR-006d | 归档路径格式 `binance/{product_line}/{symbol}/{YYYY}/{MM}/{DD}/{event_type}.parquet` | TC-017 |
| AC-029 | FR-008 | kafkax topic = `binance.market.{product_line}.{event_type}` | TC-018 |
...
| AC-034 | FR-009 | go.mod natsx/redisx/kafkax/postgresx/taosx/clickhousex/ossx/gin 均为 direct | TC-022 |
...
| BR→验证覆盖率 | — | 9/9 | 100% | — |
| AC→验证覆盖率 | — | 47/47 | 100% | — |
| 实现状态 | — | 0/15 FR | 0% | runtime 门禁尚未全 PASS；不得将 FR-009 计为已实现 |
...
| 2026-06-21 | v2.1.1 | 修正 FR/AC/TC 错位、clickhousex direct dependency 追溯、BOUNDARY-GATES §7/§8/§9/§11 引用；FR-009 从 implemented 回退为 runtime pending，防止未通过脚本被计为完成。 | ZoneCNH |
