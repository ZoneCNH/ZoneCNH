...
| NFR-S12 | ossx 归档完整性：ETag 验证通过后才执行 taosx 删除 | 数据完整性 | TC-016 |
...
|-------|-----------|------------|:--------:|---------|:----:|
| TC-001 | FR-001 | — | 单元（mock JetStream） | durable consumer 订阅 binance.market.> | ⬜ |
| TC-002 | FR-001, FR-002 | BR-001, BR-003 | 集成（处理失败→NakWithDelay） | handler 报错→NakWithDelay；未 Ack | ⬜ |
| TC-003 | FR-003 | BR-004 | 单元（缺字段→Nak） | 缺 symbol/exchange_time→ManualNak；无下游调用 | ⬜ |
| TC-004 | FR-003 | BR-004 | 单元（非法 product_line→Nak） | 非法枚举→ManualNak；无存储调用 | ⬜ |
| TC-005 | FR-004 | BR-002 | 单元（首次 SetNX→false） | SetNX 返回 true → 进入存储管线 | ⬜ |
| TC-006 | FR-004 | BR-002 | 单元（重复 SetNX→true，跳过写入） | SetNX 返回 false → Ack 并跳过，taosx.Write 未调用 | ⬜ |
...
| TC-014 | FR-007 | — | httptest（无效 API key → 401） | Authorization 无效→401 | ⬜ |
| TC-015 | FR-007 | — | httptest（超限 → 429） | 令牌桶耗尽→429 + Retry-After | ⬜ |
| TC-016 | FR-008 | BR-007 | 单元（PutObject 失败→taosx 未删除） | ossx.PutObject 报错→taosx.Delete 未调用 | ⬜ |
| TC-017 | FR-008 | BR-007 | 单元（路径格式验证） | key=binance/{line}/{symbol}/{YYYY}/{MM}/{DD}/{type}.parquet | ⬜ |
| TC-018 | FR-009 | BR-006, BR-009 | CI gate（cs 包/client 包 import 检查） | grep internal/client\|internal/cs 零匹配 | ⬜ |
| TC-019 | FR-009 | BR-008 | CI gate（go.mod 合规检查） | gin/ossx/natsx/redisx/taosx/postgresx/kafkax/clickhousex 均为 direct | ⬜ |
| TC-020 | FR-002 | — | 单元（SIGTERM 优雅关闭） | consumer 收 SIGTERM，完成当前 Ack 后关闭，不丢弃 in-flight 消息 | ⬜ |
| TC-021 | FR-002 | — | 单元（ack_wait 超时重投） | 处理超 ack_wait 未 Ack → JetStream 重投；redisx SetNX 幂等过滤 | ⬜ |
| TC-022 | FR-007a | — | httptest（/api/v1/analytics/vwap + top-movers + correlation） | clickhousex mock 返回聚合结果；clickhousex 不可达→503 | ⬜ |
| TC-023 | FR-010 | BR-008 | 集成（clickhousex ETL：taosx Query → 聚合 → InsertBatch） | 5 分钟 ETL 周期，InsertBatch 写入 1m_ohlcv / 5m_vwap | ⬜ |
| TC-024 | FR-010 | — | 单元（clickhousex 不可达→ETL 跳过，实时 API 不受影响） | InsertBatch 报错→warn+跳过；ticks API 正常返回 | ⬜ |
| TC-025 | FR-011 | — | 单元（redisx SetNX 获取锁 → 启动 ETL scheduler） | SetNX 返回 true → scheduler.Start 被调用 | ⬜ |
| TC-026 | FR-011 | — | 单元（lease 续期失败 → 停止任务；正常关闭 → Del 主动释放） | Expire 报错→停止 ETL；Shutdown→Del 被调用 | ⬜ |
...
| AC ID | AC 描述 | 验证方式 |
|-------|---------|----------|
| AC-001 | durable consumer 绑定名称 `binance-server`，进程重启后从上次 Ack 位置继续消费 | TC-001: mock 验证 Subscribe 使用 Durable option |
| AC-002 | ManualAck — Handle 成功后 Ack；Handle 失败后 NakWithDelay(5s) | TC-002: mock handler 失败→验证 NakWithDelay |
| AC-003 | consumer 不持有任何 client 接口，不导入 `internal/client` 或 `internal/cs` | TC-018: CI gate grep 零匹配 |

### FR-002: Consumer Lifecycle

| AC ID | AC 描述 | 验证方式 |
|-------|---------|----------|
| AC-031 | server 收到 SIGTERM 时，完成当前处理中消息的 Ack 后才关闭 consumer，不丢弃 in-flight 消息 | TC-020: 注入 SIGTERM → 验证 consumer 等待 Ack 后关闭 |
| AC-032 | consumer 处理超过 `ack_wait` 仍未 Ack → JetStream 自动重投；redisx SetNX 幂等过滤重复 | TC-021: mock ack_wait 超时 → 验证重投 + SetNX 幂等过滤 |

### FR-003: Envelope Validation

| AC ID | AC 描述 | 验证方式 |
|-------|---------|----------|
| AC-004 | envelope 缺必填字段（symbol / exchange_time / product_line）→ ManualNak（解析失败不重投） | TC-003: 构造不完整 JSON → 验证 Nak，无下游调用 |
| AC-005 | 不支持的 product_line 值 → ManualNak | TC-004: 非法 product_line → Nak |
| AC-006 | 合法 envelope 通过验证，进入幂等检查管线 | TC-003 负向验证反向确认 |

### FR-004: Idempotent Acceptance

| AC ID | AC 描述 | 验证方式 |
|-------|---------|----------|
| AC-007 | 首次消息：redisx SetNX 返回 true → 继续处理（非重复） | TC-005: mock SetNX 返回 true |
| AC-008 | 重复消息（相同 hash key 已存在）：SetNX 返回 false → Ack 并跳过，不写 taosx | TC-006: mock SetNX 返回 false → taosx.Write 未调用 |
| AC-009 | Redis 不可达时 → 返回 error，consumer NakWithDelay | TC-005: mock SetNX 返回 err → NakWithDelay |

### FR-005: Multi-Store Write

| AC ID | AC 描述 | 验证方式 |
|-------|---------|----------|
| AC-010 | taosx.WriteTick 使用 symbol+product_line 生成子表名，自动创建子表 | TC-007: mock 验证 WriteWithAutoCreate 参数 |
| AC-011 | taosx.WriteBatch 合并多条消息为一次网络往返（非循环 WriteTick） | TC-008: benchmark mock 验证 WriteBatch 被调用 |
| AC-012 | taosx 不可达时 → error，consumer NakWithDelay | TC-007: mock 注入 error |
| AC-013 | postgresx.UpsertSymbol ON CONFLICT DO UPDATE，幂等插入 | TC-009: 同 symbol 插入两次不报错 |
| AC-014 | postgresx.RecordClockOffset 每分钟采样写入 binance_clock_offsets 表 | TC-009: mock 验证 INSERT 被调用 |
| AC-015 | postgresx.UpdateIngestStatus 更新 last_seq，用于 gap fill 检测 | TC-009: mock 验证 seq 参数 |

### FR-006: kafkax Dispatch

| AC ID | AC 描述 | 验证方式 |
|-------|---------|----------|
| AC-016 | topic = `binance.market.{product_line}.{event_type}`，与 natsx subject 格式一致 | TC-010: mock 验证 topic 字符串 |
...
| AC ID | AC 描述 | 验证方式 |
|-------|---------|----------|
| AC-019 | GET /api/v1/market/ticks 从 taosx 查询，支持 symbol + time range + limit | TC-012: httptest + mock taosx |
| AC-020 | GET /api/v1/market/depth/:symbol 从 redisx 读取最新深度快照，P99 < 1ms | TC-013: mock redisx 验证 key 格式 |
| AC-021 | 无效 API key → 401 | TC-014: httptest 验证状态码 |
| AC-022 | 超限（>1000 req/min per key）→ 429 + Retry-After header | TC-015: mock 令牌桶耗尽 |
| AC-023 | GET /readyz 三组件（taosx / redisx / postgresx）任一断连 → 503 | TC-012: mock taosx 断连 → 503 |
| AC-024 | 所有响应统一 JSON，错误使用 `{"error":"...","code":"..."}` 结构 | TC-012~015: 验证响应体格式 |

### FR-007a: clickhousex Analytics API

| AC ID | AC 描述 | 验证方式 |
|-------|---------|----------|
| AC-033 | GET /api/v1/analytics/vwap 从 clickhousex 返回跨符号 VWAP 排名 | TC-022: mock clickhousex 返回聚合结果 |
| AC-034 | GET /api/v1/analytics/top-movers 返回涨幅/跌幅 top N | TC-022: 同上 |
| AC-035 | clickhousex 不可达时 analytics API 返回 503；实时 API（ticks/bars/depth）不受影响 | TC-022: mock clickhousex error → 503；ticks 正常 |

### FR-008: ossx Archival

| AC ID | AC 描述 | 验证方式 |
|-------|---------|----------|
| AC-025 | 每日定时归档 cutoff（now - RetentionDays）之前的 taosx 数据 | TC-017: 验证 taosx 查询参数含 cutoff |
| AC-026 | 先 ossx.PutObject + 验证 ETag，成功后才 taosx.Delete（先写冷再删热） | TC-016: mock PutObject 失败→Delete 未调用 |
| AC-027 | 归档路径 `binance/{product_line}/{symbol}/{YYYY}/{MM}/{DD}/{event_type}.parquet` | TC-017: 验证 key 字符串格式 |

### FR-009: Boundary Enforcement

| AC ID | AC 描述 | 验证方式 |
|-------|---------|----------|
| AC-028 | server 源码无 `internal/client` 或 `internal/cs` 导入 | TC-018: CI gate grep 零匹配 |
| AC-029 | go.mod 中 gin / ossx / clickhousex 为 direct 依赖；redisx/kafkax/natsx/postgresx/taosx 为 direct 依赖 | TC-019: go mod 检查 |
| AC-030 | BOUNDARY-GATES §5（cs 包禁止）+ §6（同进程禁止）CI gate 全 PASS | TC-018: 本地 PASS |

### FR-010: clickhousex OLAP Storage

| AC ID | AC 描述 | 验证方式 |
|-------|---------|----------|
| AC-036 | ETL scheduler 每 5 分钟从 taosx 聚合写入 clickhousex（1m_ohlcv / 5m_vwap / 15m_stats） | TC-023: mock taosx 查询 + InsertBatch 调用验证 |
| AC-037 | clickhousex InsertBatch 失败 → error 日志 + 告警 + 跳过本批次（下周期重试） | TC-024: InsertBatch 报错→ warn + 实时 API 正常 |
| AC-038 | 启动时 market_binance 库不存在 → 自动 DDL 建库建表 | TC-023: mock DDL 执行验证 |

### FR-011: Distributed Coordinator Lock

| AC ID | AC 描述 | 验证方式 |
|-------|---------|----------|
| AC-039 | redisx SetNX("lock:binance:coordinator", instanceID, 30s) 成功 → 启动 ETL + 归档 scheduler | TC-025: mock SetNX true → scheduler.Start 被调用 |
| AC-040 | lease 续期失败 → 停止 ETL + 归档；正常关闭 → Del 主动释放锁 | TC-026: mock Expire 失败→Stop；Shutdown→Del 被调用 |
