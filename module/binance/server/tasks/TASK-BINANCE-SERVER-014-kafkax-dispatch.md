...
Kafka topic 格式：`binance.{product_line}.{event_type}.v1`。

注意：Kafka topic 不与 natsx subject 同名；natsx subject 仍为 `binance.market.{product_line}.{event_type}`。
...
**FR-DISP-001**: Kafka topic 名称格式使用 canonical Kafka v1 命名：`binance.{product_line}.{event_type}.v1`。
