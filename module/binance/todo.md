...
| P1-08 | 本轮修复（`fix/todo-cleanup-batch` branch） | `DispatchRetryBackoffs` env 桥接（`FOUNDATIONX_BINANCE_DISPATCH_RETRY_BACKOFFS`）+ `parseDurationList` + 3 个 test |
| P2-05 | main 已修（gocyclo nolint 全部移除） | `RunStandalone`/`buildStorage`/`detectGapByEventType`/`UpsertEntries` 已重构或不再触发警告 |
| P3-01 | 本轮修复 | 删除 `validateAndApply` 死代码（零调用方） |
| P3-02 | 本轮修复 | 抽取 `dropOldestAndSend` helper，去重 Dispatch vs run forwarder 通道满逻辑 |
| P3-04 | 本轮修复 | `topnOnce`/`incrementalOnce` sync.Once 保护 `topnCh`/`incrementalCh` 懒初始化 |
| P3-06 | 本轮修复 | `AIMDConfig` + `DefaultAIMDConfig()`，6 个 AIMD 常量可配置（零值向后兼容） |
| P3-07 | 本轮修复 | `.env.example` 前缀 `XGO_BINANCE_*` → `FOUNDATIONX_BINANCE_*` + 注释订正（`XGO_BINANCE_DISPATCHER` bare env 例外保留） |
...
| P3-09 | 本轮修复 | env.example 加注释说明 `ReconcileCronHour` 代码 fallback=4 vs 示例=2 的设计意图（低峰错峰） |
...
**待合入**：本轮 8 项代码修复在 `fix/todo-cleanup-batch` branch（本地未 commit/push，待用户授权提交策略）。
