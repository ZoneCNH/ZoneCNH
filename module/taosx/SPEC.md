...
| Layer | L2 存储适配器 |
| Module-Version | v0.1.0 MVA |
| Related | `CONSTITUTION.md`, `ARCHITECTURE.md`, `module/FOUNDATION-DEPS.yaml`, `kernel` |

## 1. 摘要

`taosx` 是 TDengine 的 L2 存储适配器契约模块。当前版本交付 Go 侧可审计的适配器边界：配置归一化与脱敏、SQL 执行与查询契约、批量写入与 schemaless 写入契约、健康检查、可注入驱动端口和可选指标端口。

当前模块不内置真实 TDengine 驱动、连接池、STMT 批量写入实现或自动重试策略。调用方必须通过 `WithDriver` 注入实现了 `Driver` 接口的适配器，或者接受默认驱动返回不可用错误。
...
- 不实现真实 TDengine Go driver、HTTP/WebSocket 驱动封装或连接池。
