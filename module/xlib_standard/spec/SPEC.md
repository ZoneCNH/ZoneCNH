...
- 公共 API 缺少统一事实源时，下游基础库会重复定义 Config、Error、Health、Metrics、Client 和 Version，导致兼容性断裂。
- Generator 和 Harness Gate 缺少稳定规则时，渲染产物可能残留模板命名、非法边界引用或不可复现命令。
- Release Evidence 缺少 manifest 与 checksum 约束时，发布状态无法被 CI、评分管线和治理审计客观验证。
...
├── .golangci.yml
├── docs/
│   └── standard/              # Standard source documents
├── pkg/
│   └── templatex/             # Go Reference Template
│       ├── config.go
│       ├── client.go
│       ├── errors.go
│       ├── health.go
...
│       └── *_test.go
├── cmd/
│   └── goalcli/               # Evidence Runtime CLI
│       ├── audit_goal.go
│       ├── dashboard_generate.go
│       ├── fact.go
│       ├── schema_check.go
│       ├── traceability.go
│       ├── governance.go
...
├── templates/
│   └── l2/                    # L2 downstream repo templates (12 files)
├── scripts/
│   ├── render_template.sh
│   ├── check_rendered_template.sh
│   ├── check_boundary.sh
│   ├── check_l2_templates.sh
│   ├── generate_manifest.sh
│   └── release_check.sh
├── contracts/                 # Cross-domain contracts (4 + 12 internal)
├── testdata/
├── examples/
└── testkit/
```
...
| 日期 | 版本 | 变更内容 | 作者 |
|------|------|----------|------|
| 2026-06-18 | v1.0.0 | 结构修复：对齐 SPEC-TEMPLATE 23 节英文标题与评分 metadata 字段 | Codex |
