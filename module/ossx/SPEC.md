...
- 不做业务领域的上传工作流编排（upload workflow 由各业务服务自行实现，ossx 只提供单对象操作原语）
- 不做云厂商 SDK 类型的公开暴露（provider SDK 类型封装在 `adapters/s3/` 等 adapter/internal 层）
- 不做配置加载或配置解析（Config 由调用方 / composition root 构造后传入，ossx 只做模块自有配置校验）
- 不做跨云迁移编排（跨云数据迁移由平台运维层或独立迁移工具负责，ossx 只提供单云 BlobStore 原语）
