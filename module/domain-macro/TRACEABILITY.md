...
|-------|------|----------|--------------|--------|
| BR-MAC-001 | IsVisibleAt 必须 fail-closed：缺失 AvailableAt 的点不可见 | TC-MAC-001, TC-MAC-002 | TC-MAC-001 缺失 AvailableAt 拒绝断言；TC-MAC-002 IsVisibleAt 返回 false | ✅ | |
| BR-MAC-002 | FilterMacroPointsForBacktest 必须拒绝缺失 AvailableAt 的点，避免前视偏差 | TC-MAC-001 | TC-MAC-001 Validate 失败拒绝；Property 测试随机时间组合验证 | ✅ | |
| BR-MAC-003 | MacroInformationSet 构造器 copy-on-write：getter 返回 slice 副本 | TC-MAC-004 | TC-MAC-004 copy-on-write 断言；Race 测试并发读取 | ✅ | |
| BR-MAC-004 | 同一 DecisionTime + 同一输入数据 → MacroInformationSet 输出 deterministic | TC-MAC-003 | Fuzz 测试随机 MacroPoint 集合验证确定性；TC-MAC-003 revision selection 确定性 | ✅ | |
| BR-MAC-005 | DataFreshnessSec 规则：无可见点时返回 -1 或特殊值；未来数据拒绝 | TC-MAC-001, TC-MAC-002 | TC-MAC-001/TC-MAC-002 无可见点时返回 -1 | ✅ | |
| BR-MAC-006 | provider DTO 不得污染 domain Public API | TC-MAC-007 | TC-MAC-007 provider DTO 边界检查；lint：domain 原始值不暴露外键 DTO | ✅ | |
