...
- **模块目录化结构**：管线 `goal/spec/design/plan/tasks/prompt/test/review/release/retrospective`（S1-S11）+ 横切 `schema/matrix/gate/evidence/registry`；Goal 文档位于 `goal/goal.md`；Code 在 `/home/{module}`；`.config/goal/` 为跨模块控制面 SSOT，模块级横切目录为本地投影
