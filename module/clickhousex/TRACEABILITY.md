...
|----|-------------|----------|----------|------|--------|
| BR-001 | 连接池大小默认 10，最大 100，通过 Config 配置 | - | AC-018, Config.Validate() | ✅ | |
| BR-002 | 批量写入使用原生 batch insert 协议，不使用拼接 SQL | TC-003 | AC-019, TC-003 | ✅ | |
| BR-003 | Exec / Query 的 args 使用参数化绑定，禁止 SQL 拼接 | TC-001 | AC-020, TC-001 | ✅ | |
| BR-004 | 连接断开后自动重试 3 次（指数退避），超过后返回 `ErrConnectionLost` | TC-002 | AC-021, TC-002 | ✅ | |
| BR-005 | Health() 必须是幂等的、无副作用的 | TC-006 | AC-022, TC-006 | ✅ | |
| BR-006 | 所有操作必须接受 `context.Context`，支持取消和超时 | - | AC-023, FR-002/FR-003/FR-004 WHEN ctx 取消 | ✅ | |
| BR-007 | 错误消息格式：`"clickhousex: <operation>: <detail>"` | - | AC-024, go test 错误消息断言 | ✅ | |
| BR-008 | 可观测指标必须包含 table 标签（写入操作）或 query 标签（查询操作） | - | AC-025, metrics 测试 | ✅ | |
| BR-009 | Close() 必须是幂等的，多次调用不 panic | TC-007 | AC-015, TC-007 | ✅ | |
| BR-010 | InsertBatch 不自动建表，表不存在时返回明确错误 | - | AC-011, FR-004 WHEN table 不存在 | ✅ | |
| BR-011 | ClickHouse Nullable 类型映射到 Go 指针类型 | TC-004 | AC-013, TC-004 | ✅ | |
| BR-012 | ClickHouse Decimal 类型映射到 `shopspring/decimal` 或 `apd.Decimal` | - | AC-026, 类型映射表测试 | ✅ | |
