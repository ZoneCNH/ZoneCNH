...
> **⚠️ 阅读说明**：下表百分比表示「**追溯链闭合率**」（FR/BR/NFR/AC/TC/Task 之间是否两两挂钩），**不是「实现完成度」或「测试通过率」**。100% 闭合仅代表文档追溯完整，不代表运行时代码已实现、测试已通过或门禁已达标。运行时真实状态以 `/home/testkitx` 的 CI 结果为准（当前 Prompt 阶段评分 95/100，门禁 98 未过，详见 `prompt/scores/`）。
