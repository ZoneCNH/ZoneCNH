...
|----|-------------|----------|----------|------|--------|
| BR-001 | Source 节点必须声明输出数据类型 | TC-FLX-002 | TC-FLX-002 OutputType() 非空断言 | ✅ | |
| BR-002 | 至少一个 Sink 必须标注为 primary | TC-FLX-005 | TC-FLX-005 primary Sink 存在性检查 | ✅ | |
| BR-003 | DAG 中不得有循环依赖 | TC-FLX-001 | TC-FLX-001 循环检测断言 | ✅ | |
| BR-004 | Sink at-least-once 语义不可降级为 at-most-once | TC-FLX-005 | TC-FLX-005 DeliveryReceipt ACK 断言 | ✅ | |
| BR-005 | 背压策略必须显式配置，不允许隐式默认 | TC-FLX-007 | TC-FLX-007 背压配置非空检查 | ✅ | |
...
| FR 有 AC 覆盖 | 10/10 (100%) | |
| FR 有 TC 覆盖 | 10/10 (100%) | |
| BR 总数 | 5 | BR-001 ~ BR-005 |
| BR 有 TC 覆盖 | 5/5 (100%) | |
| NFR 总数 | 7 | NFR-001 ~ NFR-007 |
| AC 总数 | 10 | AC-FLX-001 ~ AC-FLX-010 |
| TC 总数 | 10 | TC-FLX-001 ~ TC-FLX-010 |
