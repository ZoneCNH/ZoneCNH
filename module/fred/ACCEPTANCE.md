...
外，其余 runtime 验收项在拿到 `/home/fred` 实现证据前均保持 `Pending`。
...
| V-005 | `cd /home/fred && go test ./pkg/fredx/...` | FRED SDK 的参数编码、分页、限流、重试和错误分类测试通过。 | Pending |
| V-006 | `cd /home/fred && go test ./internal/domain/... ./internal/client/...` | provider 响应可稳定转换为 `domain_macro` 模型。 | Pending |
| V-007 | `cd /home/fred && go test ./internal/server/... -run Idempotency` | backfill、observation、event 重放保持幂等。 | Pending |
...
| AC-002 | 配置扫描证明 `module/fred/` 和 `/home/fred` 不包含 dev secret 值。 | FR-002 | V-002、secret scan | Pending |
...
| --- | --- | --- | --- | --- |
| TC-001 | `pkg/fredx` 参数编码、错误分类、分页、限流和退避重试。 | FR-003、FR-013 | `cd /home/fred && go test ./pkg/fredx/...` | Pending |
| TC-002 | provider response 到 `domain_macro` 转换。 | FR-005、BR-001、BR-008 | `cd /home/fred && go test ./internal/domain/... ./internal/client/...` | Pending |
| TC-003 | observation、event、backfill job 幂等。 | FR-004、BR-002 | `cd /home/fred && go test ./internal/server/... -run Idempotency` | Pending |
| TC-004 | raw-first、多存储写入和 read model 集成闭合。 | FR-006、FR-007、FR-008、FR-010、FR-012 | `cd /home/fred && FRED_DEV_CONFIG=/home/ZoneCNH/sre/secrets/env/dev.md go test ./internal/integration/...` | Pending |
| TC-005 | Redis 清空后可重建缓存。 | FR-009、BR-006 | `cd /home/fred && go test ./internal/server/... -run RedisRebuild` | Pending |
