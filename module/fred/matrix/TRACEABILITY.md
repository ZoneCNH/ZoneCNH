...
Source: [spec/SPEC.md](../spec/SPEC.md) | Goal: GOAL-FRED-001 | Status: Planned (Production Target)
...
| -- | -------- | -- | -------- | ---- | ---- |
| FR-001 | `fred-client`/`fred-server` 双服务启动、health、version | AC-001 | TC-006 | TASK-FRED-001 | Planned |
| FR-002 | `configx` 从 `sre/secrets/env/dev.md` 映射配置 | AC-002 | TC-006 | TASK-FRED-001 | Planned |
| FR-003 | FRED 全信息域采集 + `spec/SPEC.md` §5.1 全端点矩阵/核心指标包覆盖 + 分页限流重试 | AC-006 | TC-001 | TASK-FRED-CLIENT-001 | Planned |
| FR-004 | backfill / incremental / series sync / revision scan | AC-003, AC-006 | TC-003, TC-005 | TASK-FRED-SERVER-001 | Planned |
| FR-005 | 转换为 `domain_macro` 并记录信息集时间 | AC-006 | TC-002, TC-007 | TASK-FRED-CLIENT-002 | Planned |
| FR-006 | OSS raw 先归档再规范化写入 | AC-003 | TC-004 | TASK-FRED-SERVER-001 | Planned |
| FR-007 | TDengine observation 写入和查询 | AC-003 | TC-004 | TASK-FRED-SERVER-001 | Planned |
| FR-008 | Postgres metadata / idempotency / checkpoint | AC-003 | TC-003, TC-004 | TASK-FRED-SERVER-001 | Planned |
| FR-009 | Redis cache / lock / rate bucket / cursor | AC-004 | TC-005 | TASK-FRED-SERVER-001 | Planned |
...
| -- | -------- | -- | -------- | ---- | ---- |
| BR-001 | 不暴露 provider DTO，对外只出服务 API / Kafka 事件 / `pkg/fredx` / `domain_macro` | AC-007 | TC-002, TC-008 | TASK-FRED-CLIENT-002 | Planned |
| BR-002 | 相同 provider / series / period / vintage 写入幂等 | AC-003 | TC-003 | TASK-FRED-SERVER-001 | Planned |
| BR-003 | `available_at` 是 no-lookahead 判定依据，晚于 `released_at` 时下游只能在 `available_at` 后使用 | AC-006 | TC-007 | TASK-FRED-CLIENT-002 | Planned |
| BR-004 | Kafka 是 durable business event，NATS 只承载 ingest/control plane | AC-005 | TC-006 | TASK-FRED-CLIENT-001, TASK-FRED-SERVER-001 | Planned |
| BR-005 | Postgres checkpoint 成功推进前，backfill job 不得进入 completed | AC-003 | TC-003, TC-004 | TASK-FRED-SERVER-001 | Planned |
| BR-006 | Redis 与 ClickHouse 均为可重建派生层，不作为唯一权威源 | AC-004 | TC-005 | TASK-FRED-SERVER-001 | Planned |
| BR-007 | OSS raw 路径包含 provider、endpoint、日期、job_id、content hash | AC-003 | TC-004 | TASK-FRED-SERVER-001 | Planned |
| BR-008 | `macro_data` 不依赖 `fred/internal/*`、provider DTO 或私有存储表 | AC-007 | TC-002, TC-008 | TASK-FRED-001 | Planned |
| BR-009 | `fred` 不实现 `ms_brain` 的 M/S 状态机、交易许可、仓位折扣或策略判断 | AC-009 | TC-009 | TASK-FRED-SERVER-002 | Planned |
| BR-010 | 全量采集按 `spec/SPEC.md` §5.1 全端点矩阵进行跨入口交叉校验 | AC-010 | TC-010 | TASK-FRED-CLIENT-001, TASK-FRED-SERVER-001 | Planned |
...
| AC | 验收摘要 | 覆盖需求 | 状态 |
| --- | --- | --- | --- |
| AC-001 | 双服务启动、health、version、readiness 正确 | FR-001, FR-013 | Planned |
