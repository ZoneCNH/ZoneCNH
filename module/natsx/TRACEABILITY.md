...
| FR-006 | JetStream.AddStream | 创建、幂等、冲突配置均有测试 | TC-003 | TASK-NATSX-003 | ✅ |
| FR-007 | JetStream.AddConsumer | 创建、幂等、冲突配置均有测试 | TC-003 | TASK-NATSX-003 | ✅ |
...
| NFR-009 | Observability contract | `foundationx_nats_*` 指标和结构化日志字段有测试 | TC-009 | TASK-NATSX-009 | ✅ Canonical `foundationx_nats_*` metrics and metadata-only structured logs covered |
...
| BR-004 | Handler latency | 订阅 handler 快速返回/异步化约束有测试或示例 | TC-001 | TASK-NATSX-010 | ✅ |
