...
- 规则完整 WHEN/THEN 详见 `../FR-DETAIL.md`；本文件只保留职责入口和块级语义。

## 2. 覆盖职责（FR 摘要）

### 2.1 Standard Source

> 权威来源：`../FR-DETAIL.md` FR-001..FR-008。
...
### 2.2 Debt Governance Runtime

> 权威来源：`../FR-DETAIL.md` FR-033..FR-039。
...
| FR-038 | IMPL 类技术债规则 | P0 | PANIC_RUNTIME 等规则验证 |
| FR-039 | SEC 类技术债规则 | P0 | 安全合规规则验证 |

## 3. 规则正文与治理契约

### 3.1 核心铁律（Iron Rules，BR-001..BR-007）

> 编号别名：`BR-NNN` 为对外引用的业务规则编号；`RULE-CORE-NNN` 仅保留给 enforcer / registry.yaml 源码引用。旧内部分类编号已废弃；`TRUTH-NNN` 只作为 `analysis/governance.md` §7 的同义引用表。
...
> 本表为块级映射，行级 RULE→FR→TC 映射由上游 `registry.yaml` + `goalcli trace-coverage` 维护；本地追溯口径见 `../TRACEABILITY.md` 的块级缺口声明。

| RULE 前缀 | 主要覆盖 FR | 覆盖说明 |
|-----------|-------------|----------|
| RULE-CORE | FR-001 / FR-007 | 7 条铁律 / 基本真理同义表见 `analysis/governance.md` §7 |
...
| RULE-TEST | FR-020 / xlib-TC-001..017 | 测试分层、覆盖率、race gate |
| RULE-DOCS | FR-008 / INDEX.md | ADR、文档入口 |
| RULE-ARCH | FR-004 | 层级治理 |
| RULE-DOMAIN | FR-005 / FR-047..FR-052 | 仓库治理、采纳状态机、下游同步 |
| RULE-SEC | FR-013 / FR-014 | 配置脱敏、secret policy、日志脱敏 |
...
1. **依赖方向**：高层只能指向低层，层级模型详见 `analysis/governance.md` §3.6。
2. **L3 私有边界**：L3 业务系统不公开、不开源，公开库不得包含业务语义。
3. **配置显式传入**：不得隐式读取 `<secret-store-path>`。
4. **日志脱敏**：不得输出 secret/token/password/private key/连接凭据。
5. **单一执行面**：`cmd/goalcli` 是唯一机器执行面，拒绝第二套并列执行面。
6. **证据不可删**：禁止删除失败 Evidence。
...
- 弱事实不得升级为强事实：`registered`、`baseline_scanned`、`dry_run_ready`、`artifact_exists` 和 `CHECK_STATUS=passed` 都不是 release-ready 证明。
- Debt Governance 只在上游 gate / registry / evidence 中生效；本仓库不运行 debt-scan，也不声明 scan 通过。
- 规则数量、优先级和 enforcer 覆盖率只作为 pinned 快照事实；未来上游变更必须刷新 `COVERAGE-MANIFEST.md`。

## 5. 与其他子分析的交叉引用

| 主题 | 位置 |
|------|------|
| Go 模板中的配置、错误和安全约束 | `analysis/template.md` §3、§4 |
| Harness / Evidence 执行语义 | `analysis/runtime.md` §3、§4 |
| AdoptionStatus 与 truth-state | `analysis/governance.md` §3.2、§4.2 |
| 同一 SSOT 内硬冲突 | `../CONFLICT-LEDGER.md` |
| 快照现实边界 | `../SNAPSHOT-BOUNDARY.md` |

## 6. TC / EC 命名空间

本文件不直接定义 Go test 样板。规则类 FR 的直接测试由上游 registry / enforcer / harness gate 覆盖；跨文档引用测试用例时统一使用 `analysis/runtime.md` §6 的 `xlib-TC-001..xlib-TC-017` 命名空间。
