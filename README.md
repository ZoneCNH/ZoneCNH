...
- [natsx](https://github.com/ZoneCNH/natsx) — NATS 内部通信模块（v1.0.3 remote tag 已存在，GitHub Release v1.0.3 待补；Spec v1.2.0；Core NATS / JetStream、Drain/reconnect/degraded health、canonical `FOUNDATIONX_NATS_*` 配置和真实 dev auth live gate 已验证；FR-009/010 runtime adapter PR #17 已合并，JetStream IngestAdapter 域适配契约 pkg/natsx/ingest 解耦 binance PR-007c/d；正式四源 98+ arbiter 与生产 TLS gate 待补，BLK-001/BLK-002 open；非 factory） `公开`
