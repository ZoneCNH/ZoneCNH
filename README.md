...
> **公开投影口径**：版本 / release / factory 状态以 `.foundationx/status/index.json` + `.foundationx/blockers.json` 和 GitHub Release 实际证据为准；2026-06-23 核查发现 `natsx` 仅有 `v1.0.3` 远端 tag，GitHub Release `v1.0.3` 尚不存在；Foundation factory 投影不得把 tag 直接等同 Release。
>
...
=======
- [natsx](https://github.com/ZoneCNH/natsx) — NATS 内部通信模块（v1.0.3 remote tag 已存在，GitHub Release v1.0.3 待补；Core NATS / JetStream、Drain/reconnect/degraded health、canonical `FOUNDATIONX_NATS_*` 配置和真实 dev auth live gate 已验证；FR-009/010 runtime adapter PR #17 已合并；正式四源 98+ arbiter 与生产 TLS gate 待补，BLK-001/BLK-002 open；非 factory） `公开`
>>>>>>> origin/main
