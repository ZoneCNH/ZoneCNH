...
- [binance](https://github.com/xhyperium/binance) — 币安 Binance Market Data C/S Module (Spot/USDⓈ-M/COIN-M/Options)；spec v3.9.8 ✅；Runtime `v0.12.0` released；single state `48 Done / 0 Partial`（100%）；release_closeable=YES 🎉；47/47 tasks Done；P0×6 阻断闭环 + P1×6 CI gate 落地（测试分层/canary drill/depth 覆盖/版本一致性 gate）；coverage 100.0%（22/23 packages 100%, 2 kernel-level dead-code funcs remain）。 `公开`
