...
| domainx | PASS | PASS | 19 |
| domain_market | PASS | PASS | 11 |
| domain_macro | PASS | PASS | 3 |
| domain_exchange | PASS | FAIL | 22 |

关键真实发现（非伪造）：
- **domainx / domain_market**：领域值在 `NewOrder`/`NewTrade`/`NewPortfolio`/`NewPosition`/`NewExecutionReport`/`NewTDEngineCompensationTask` 构造函数中直接调用 `time.Now()`（DP-003/DP-011）→ 真实不确定性来源，需注入时钟修复（30 天分批门禁程序）。
...
- **R3 — L2.5 不确定性**：domainx/domain_market 构造函数 `time.Now()` 未修复（见上）。
...
本基线满足"快照可回滚、可验证、可重放"，但**未达**计划 §35 的 `DONE with evidence` 全绿条（identity/version/projection drift ≠ 0、consumer failures ≠ 0）。修复 R1–R5 是后续分批门禁工作，本会话未伪造其完成。
