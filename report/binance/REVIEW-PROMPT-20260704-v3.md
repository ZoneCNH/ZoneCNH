...
> **版本**: v3.0.0
> **生成日期**: 2026-07-04
> **前序版本**: v2.0.0（`report/binance/REVIEW-PROMPT-20260702.md`，1271 行）、v1.0.0（`report/binance/REVIEW-PROMPT-20260630.md`，679 行）
...
> **适用场景**: AI agent 深度审查、CI 巡检、发布前门禁、治理审计、运行时缺口复核、**20 轮独立并行 agent team 复核**
> **输出**: 结构化审查报告 + 评分卡 + 问题清单 + 运行时缺口核验 + （本版新增）20 轮独立结果一致性矩阵
...
对 `module/binance` 模块执行端到端深度审查，覆盖 **15+1 个治理维度 + 管线 8 阶段 + 运行时缺口 58+7 项**，产出：

1. **评分矩阵**（16 维度 × 满分 100，含加权综合分 + 双口径分）
2. **红线清单**（违反硬约束的 CRITICAL 问题）
3. **修复优先级排序**（P0 阻塞发布 / P1 影响治理 / P2 优化建议）
4. **发布就绪判定**（双口径：规格口径 release_closeable + 运行时缺口口径）
5. **运行时缺口核验**（GAP-E1~E58 抽样源码验证 + 漏洞链确认 + N1-N7/GAP-E59~E65 新缺口核验）
6. **对比上轮审查**（REVIEW-20260630 综合 97 分 + DEEP-ANALYSIS-20260704 新发现，标注变化趋势）
7. **（独立 reviewer 专用）** 严格按下方"20 轮独立复现协议"输出模板产出，供后续跨 20 份报告聚合
...
>
> **T0-2【v3.0 新增，本轮独立 reviewer 必须复现，非转述】main 分支可编译性**：`git worktree add /tmp/review-wt origin/main && cd /tmp/review-wt && go build ./...` 于**干净副本**（非本地脏工作区）验证是否仍报 `internal/server/assembly/storage.go:313: unknown field runtime in struct literal of type storageAssembly`。若已修复需注明修复 PR 号；若仍失败需作为 CRITICAL 记录（此前 DEEP-ANALYSIS-20260704 §4.2 已独立复现一次，本轮须重新验证是否仍然成立）。
>
> **T0-3【v3.0 新增】NATS subject 段数**：`grep -n "Subject" /home/workspace/binance/internal/server/consumer/consumer.go` 与 `grep -n "func Subject" -A6 /home/workspace/binance/internal/client/publisher/publisher.go`，核对 client 发布 subject 的段数（`binance.market.{pl}.{et}.v1` = 5 段）与 server `FilterSubject`/`Stream.Subjects`（`binance.market.*.*` = 4 段）是否一致。NATS `*` 精确匹配 1 个 token，`>` 才匹配多段——若仍是 4 段 vs 5 段，须在报告中给出**明确 YES/NO 结论**（"两者不匹配"），不得再用"需实测确认"含糊带过。
>
> **T0-4【v3.0 新增】release_closeable 内部自相矛盾**：`grep -n "release_closeable" module/binance/matrix/TRACEABILITY.md` 应同时出现顶部 `YES` 声明（约 L8/L87/L114/L118）与 §PRG 表 `PRG-006 | ... | Partial`（约 L96）。若两者仍同时存在于同一文件，为 CRITICAL（文档自相矛盾，非仅跨文档口径不一致）。
...
- **07-04 新发现的增量问题【v3.0 新增】**：main 分支编译失败（`storageAssembly.runtime` 字段缺失）、NATS subject 段数不匹配、ACK 时序缺陷（`MarkDurable()` 先于落库）、UM/CM/Options connector 未接入生产启动路径、TaosWriter 不支持 funding_rate/mark_price、OLAP 内存窗口非文档所述 ETL、Retention 仅覆盖 spot
...
## Part 16: DEEP-ANALYSIS-20260704 新发现核验【v3.0 新增】

本 Part 要求独立复现 `report/binance/DEEP-ANALYSIS-20260704.md` §5 提出的 7 项新发现（N1-N7）与 §9 十轮自审的 3 处修正，禁止仅转述该报告结论——必须重新执行命令/读取源码得出**独立**判断。
...
```bash
grep -oE 'GAP-E[0-9]+' /home/workspace/ZoneCNH/plans/binance/RUNTIME-GAP-MATRIX.md | sort -u | tail -3
grep -n "runtime.*field\|storageAssembly\|subject.*不匹配\|MarkDurable\|ErrUnsupportedEventType" /home/workspace/ZoneCNH/plans/binance/RUNTIME-GAP-MATRIX.md
```

若矩阵最大编号仍为 `GAP-E58` 且关键词零命中，确认 N1-N7 尚未并入，建议编号 `GAP-E59~E65` 仍然有效；若已并入需在报告中注明新编号，避免重复记录。
...
```
单次审查：      report/binance/REVIEW-{YYYYMMDD}.md
20轮独立审查：  内部产出（reviewer_id=N），不直接落盘到 report/binance/；
                聚合方负责将 20 份结果汇总为唯一的
                report/binance/REVIEW-{YYYYMMDD}-20ROUND-CONSENSUS.md
```
...
## 20 轮独立复现协议【v3.0 新增，仅供 agent team 并行执行时使用】

> 本节仅在"20 轮独立并行审查"场景下生效。若为单次人工/单 agent 审查，可忽略本节，按上方 Part 0-16 常规执行。
...
1. **禁止假设其他 reviewer 的结论**——你是 20 个独立 reviewer 之一（reviewer 编号将在任务中告知），你的报告不得引用"其他审查"或"已知共识"，只能基于你自己在本次执行中读取的文件/命令输出。
2. **固定基线快照**——所有 reviewer 必须使用相同的锚点：ZoneCNH 仓 `main@cb7161cc`、runtime 仓 `origin/main@14a30b9`。执行前应先 `git fetch` 确认未漂移；若发现锚点已变化（本仓库存在高频自动化提交），须在报告开头注明实际读取时的 HEAD SHA，不得静默使用不同基线。
3. **独立复现，不转述**——凡本 Prompt 中标注"须复现""不得转述"的检查项（T0-2/T0-3/T0-4、Part 16 全部），必须自己执行命令/读取源码得出结论，即使结论与 `DEEP-ANALYSIS-20260704.md` 一致也要基于自己的证据。

### 统一输出模板（供聚合方解析）

每个 reviewer 的报告除遵循 Part 0-16 完整结构外，须在文末追加以下机器可解析摘要块：

```yaml
reviewer_id: <1-20>
baseline_zonecnh_sha: <实际读取时的 main HEAD>
baseline_runtime_sha: <实际读取时的 origin/main HEAD>
scores:
  A_spec_structure: <0-100>
  B_traceability: <0-100>
  C_design: <0-100>
  D_code_quality: <0-100>
  E_boundary: <0-100>
  F_testing: <0-100>
  G_cicd: <0-100>
  H_security: <0-100>
  I_observability: <0-100>
  J_production_ready: <0-100>
  K_doc_consistency: <0-100>
  L_gap_coverage: <0-100>
  M_exchangeinfo_tier: <0-100>
  N_dual_baseline: <0-100>
  O_evidence_credibility: <0-100>
  P_new_findings_verification: <0-100>
  weighted_total: <0-100>
release_closeable_spec_caliber: YES|NO|PARTIAL
release_closeable_runtime_caliber: YES|NO|PARTIAL
main_branch_compiles: true|false|not_tested
nats_subject_mismatch_confirmed: true|false|not_tested
traceability_self_contradiction_confirmed: true|false|not_tested
critical_findings:
  - id: <你自己命名的编号，如 R{reviewer_id}-C1>
    summary: <一句话>
    evidence: <文件:行号 或 命令输出摘要>
new_findings_not_in_n1_n7:
  - summary: <若发现 N1-N7 之外的新问题，列在此处，供聚合方去重>
```

### 执行方式建议

- 分批派发（如每批 5 个），避免单次并发过载
- 每个 reviewer 独立的 prompt 中必须包含"你是 20 个独立 reviewer 之一，reviewer_id=N"字样，及本文件的完整路径
- 聚合方读取 20 份 `reviewer_id` 标记的报告后，按维度计算 mean/median/stdev，输出一致性矩阵与发现并集
...
7. **已知陷阱优先**：T0-1 ~ T10-1（十一个 v2.0 陷阱）+ T0-2/T0-3/T0-4（三个 v3.0 新增陷阱，见 Part 0.1）共**十四个**已知陷阱验证点须逐一确认，这些是基于历次深度分析 + 累计 30 轮自审验证（v2.0 时 20 轮 + DEEP-ANALYSIS-20260704 十轮）发现的真实问题
