...
| --- | --- |
| 日期 | 2026-06-22 |
| 分析对象 | `/home/ms_brain/` 与 `module/fred/` |
...
1. [COMPUTED][HIGH] `/home/ms_brain` 是宏观状态驱动的加密资产决策系统，其 L1 宏观层、L3 决策矩阵、L4 PIT 记忆层都需要稳定的宏观数据、发布日历、修订版本和 no-lookahead 查询能力。
...
| 证据 | 文件 | 对 `fred` 的含义 |
| --- | --- | --- |
| `ms_brain` 使用 M1-M7 宏观叙事、S1-S7 市场状态和 7x7 决策矩阵 | `/home/ms_brain/README.md` | `fred` 只供给宏观数据和版本，不承接策略矩阵 |
| L1 宏观层包含 LGIP、事件引擎；L4 记忆层要求 PIT | `/home/ms_brain/README.md` | `fred` 必须支持 PIT/as-of、发布日历和修订事件 |
| 工程状态显示框架文档完成，`framework/` 下暂无可运行代码 | `/home/ms_brain/README.md` | 当前只能先做 contract fixture，不能声称真实下游 runtime 已通过 |
| `data_sources.yaml` 声明 FRED 源、`FRED_API_KEY`、`pit_enabled: true`、`DFII10`、`T10YIE`、`DFF` 和 freshness/degrade | `/home/ms_brain/framework/config/data_sources.yaml` | `fred` 需输出 freshness/degrade 元数据和 PIT 版本字段 |
| LGIP 校验报告引用 `BAMLH0A0HYM2`、`T10Y2Y`、`ICSA` 等宏观序列 | `/home/ms_brain/framework/validation_report_LGIP_v2.1.md` | 这些序列应进入初始 contract fixture |
| `global_macro.yaml` 引用 `FYFSGDA188S`、`FDHBFRBN`、事件驱动/月度/季度/日度调度 | `/home/ms_brain/framework/config/global_macro.yaml` | `fred` 需支持财政类低频序列和 release-lag 口径 |
| PIT 架构文档说明修订会造成前视偏差，并要求 `observed_at <= trading_time` 查询 | `/home/ms_brain/framework/06_data_cortex/PIT_ARCHITECTURE.md` | `fred` 的 as-of 查询必须以可获得时间过滤 |
| TAOS PIT spec 要求 append-only revision、vintage query 和 FRED/ALFRED adapter | `/home/ms_brain/framework/06_data_cortex/02_data_specification_PIT.md` | `fred` 的 TDengine/TAOS 写入应保留 revision 和 vintage |
| 事件系统覆盖 FOMC/CPI/NFP、Before/During/After tag 和 ReduceOnly override | `/home/ms_brain/specs/market_state_engine/14_事件系统_日历_外生冲击与覆盖规则.md` | `fred` 只输出事件日历和风险标签输入，不输出策略 override 决策 |
...
| 缺口 | 影响 | 解决方案 |
| --- | --- | --- |
| `ms_brain` 目前主要是文档/spec/YAML，缺真实消费进程 | 无法做端到端 runtime 证明 | 先在 `/home/fred` 建 `MsBrainContract` fixture；待 `ms_brain` runtime 落地后补 E2E |
...
| 旧 `/home/fred` 边界脚本仍有 `Stores=None` 历史口径 | 与七类存储目标冲突 | 阶段 1 更新 boundary gate，允许目标 adapter，禁止直接 infra connection |
...
1. 在 `/home/fred` 增加 `ms_brain` integration profile fixture，至少覆盖 `DFII10`、`T10YIE`、`DFF`、`BAMLH0A0HYM2`、`T10Y2Y`、`ICSA`、`FYFSGDA188S`、`FDHBFRBN`。
