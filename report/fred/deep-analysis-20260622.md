...
- Date: 2026-06-22
- Scope: `module/fred/`, `/home/workspace/fred`, 相关架构文档、`sre/secrets/env/dev.md` 的无值匹配计数
...
当前最高优先级是把 `TRACEABILITY.md` 的规则编号漂移、`/home/workspace/fred` 的旧 `Stores=None` 门禁、配置 key mapping、API/event/schema 契约和验收证据补齐。[COMPUTED][HIGH]
...
| `module/fred/SPEC.md:14`                       | 规格声明自身是目标状态，`/home/workspace/fred` 当前仍保留旧 `Stores=None` 边界口径。                                               | [COMPUTED][HIGH] |
...
| `/home/workspace/fred/cmd/fred-server/main.go:1-4`       | 当前服务入口注释仍称 `adapter 零存储`。                                                                                  | [COMPUTED][HIGH] |
| `/home/workspace/fred/cmd/fred-server/main.go:17-20`     | 当前 `bootstrap.Build` 使用 `Stores: bootstrap.None`。                                                                   | [COMPUTED][HIGH] |
| `/home/workspace/fred/scripts/boundary-gates.sh:101-105` | 当前边界门禁仍禁止直接依赖 L2 存储适配器，规则名是 `no-storage-adapter`。                                                | [COMPUTED][HIGH] |
...
| `/home/workspace/fred` 验证                              | 当前 `./scripts/boundary-gates.sh` 9 项通过，`go test ./...` 通过；这只能证明旧骨架当前自洽，不能证明目标 C/S 服务完成。 | [COMPUTED][HIGH] |
...
`SPEC.md` 要求七类介质通过共享基座接入，`/home/workspace/fred` 当前入口和门禁仍以 `Stores=None` 和 `no-storage-adapter` 为目标。[COMPUTED][HIGH]
...
| P0     | 更新 `/home/workspace/fred` 边界门禁目标。                                 | `/home/workspace/fred/scripts/boundary-gates.sh`                                   | 门禁允许共享基座存储 adapter，禁止直接驱动、私有连接池和绕过 `configx`。             |
| P0     | 增加配置 redaction 测试。                                        | `/home/workspace/fred` tests                                                       | 缺失配置 fail fast，日志和错误不泄露值。                                             |
...
| P1     | 补 no-lookahead 和 revision fixture。                            | `/home/workspace/fred` tests                                                       | `released_at < available_at` 时，下游查询在 `available_at` 前不可见。                |
...
2. 边界迁移迭代：改 `/home/workspace/fred` 的 `Stores=None` 入口和 `no-storage-adapter` 门禁，建立共享基座接入规则。[INFERRED][HIGH]
...
6. 验收发布迭代：更新 traceability 状态和证据，跑 `/home/workspace/fred` 单元、契约、边界、集成、redaction、no-lookahead 测试。[INFERRED][HIGH]
...
- `/home/workspace/fred` 不再以旧 `Stores=None` 作为目标边界。
...
没有在本次报告中修改 `/home/workspace/fred` 代码或启动真实 dev infra，因此无法证明七类介质的运行时连接可用。[COMPUTED][HIGH]
...
最短可交付路径是先修文档追溯和契约，再改 `/home/workspace/fred` 边界门禁与服务骨架，然后用一个最小 FRED series 打通 raw、checkpoint、observation、event、read model 和 API 查询证据。[INFERRED][HIGH]
