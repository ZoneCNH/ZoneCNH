...
> 适用范围：`report/architecture-structural-analysis-20260621.md` 中原先列出的 7 项收口动作。
...
- 成功标准：每一项都已落到明确产物、依赖和退出条件，且后续文档只引用本文件的闭环结论，不再重复展开执行细节。
...
| 文件 | 需要对齐的内容 |
| --- | --- |
| `report/architecture-structural-analysis-20260621.md` | 保留结构性诊断与摘要，后续执行说明统一引用本文件的闭环结果 |
| `STATUS.md` | 阻塞项与下一步行动改为闭环摘要，不再保留旧的待办措辞 |
| `report/repo-naming-unification-20260620.md` | 只保留命名同步结果与历史投影边界，作为闭环说明，不再承载执行树 |
...
10. 相关同步文档已全部指向本文件，不再各自维护一份完整执行树或计划稿。
...
- 已将 `docs/architecture/05-foundation.md` 的主状态表收口为事实层与已发布投影层，历史名不再出现在主叙事。
- 已将 `ARCHITECTURE.md` 的主状态表同步收口，保留的历史名仅存在于既有迁移说明，不再重复扩散到状态表。
- 已补入 `docs/architecture/05-foundation.md` 的 `### 历史兼容映射（仅供迁移）`，把 `backtest_engine`、`risk_engine`、`order_engine`、`portfolio_engine` 的旧名统一收束到迁移区。
- 复核命令已通过：`git diff --check`。
- 复核检索已通过：旧名命中仅保留在迁移/兼容说明中，主状态表不再重复历史名。
