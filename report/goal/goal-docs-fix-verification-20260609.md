...
本轮 P2/P3 residual 已收敛为可验证自测或明确边界：`docs/goal/tools/self-test.sh` 覆盖正向基线与三类负向 fixture；`report/goal/` 明确保持分析/关闭证据目录，不进入 Goal runtime gate；父级 `.worktree/` 保持 ignored/local。worker-1 复核开始时目标 worktree 的 `git status --short --untracked-files=all` 无输出。
...
| 报告目录是否进入 runtime gate 边界不明                 | `report/goal/README.md` 与本报告明确报告目录是分析/关闭证据，不替代 runtime gate                      |
...
git ls-files --error-unmatch docs/goal/00-authority-map.md .config/goal/schema/rules.yaml report/goal/README.md report/goal/goal-docs-fix-verification-20260609.md
...
- `report/goal/` 下报告文件为分析/计划/关闭证据，不参与 Goal runtime gate。
