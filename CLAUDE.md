...
- **统计方法必须可复现**：用 shell one-liner 而非人工点数。示例：
  ...
  ```
- **同步检查表维护**：`STATUS.md` 文档同步检查表（≈L330-343）的 README/ARCHITECTURE/STATUS 三列必须与各文档 `grep -oP 'github\.com/ZoneCNH/[a-zA-Z0-9_.-]+' | sort -u | wc -l` 的实际 unique repo 数一致。STATUS 按域统计 domain-sum（79）与 unique（77）的差异来自 observex 双归属和 module 独立计数；不要把语言标准库误计为项目模块。
