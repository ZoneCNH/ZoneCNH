...
### 编辑前基线确认（2026-06-14 kafkax 复盘）

> 基于一次"不知道 PR #6 已重写目标文件、基于错误基线反复编辑"的 session 复盘。编辑前必须先确认当前状态。

- **`git log --oneline -5` 检查目标文件最近提交**——确认是否已有他人/其他 session 修改。禁止假设文件仍是自己上次看到的状态。
- **列出验证清单，不列变更清单**——先确定需要查什么（实际 struct 字段？panic 处理？文档内容？），验证完了再列变更清单。
- **声称"已完成"前必须 grep 验证**——对 README 或文档中的任何代码事实声称（"Config 有 X 字段""handler panic 被捕获"），用 `grep` 或 `head` 对照源码确认。禁止凭常识假设。

### 编辑策略

- **同一文件只编辑一次**：列出该文件所有需要变更的行，一次性完成
- **`Write` 前必须先确认当前基线**——禁止在未验证当前文件状态的情况下全量重写。先 `git log` 确认无他人修改，再 `Read` 确认当前内容。
...
- **校验命令不受成本约束**：`git log`、`grep`、`head`、`ls` 等单次调用成本极低（< $0.02），但不执行导致的错误修正成本极高（kafkax session：跳过 5 秒 grep → $49.52 无效编辑）。编辑文件前和声称完成前的校验命令**必须执行**，不计入成本控制范围。
