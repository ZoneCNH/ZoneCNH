...
## 5. 架构位置
基座层（L2 存储扩展）。依赖方向：仅依赖 Go stdlib + `github.com/jackc/pgx/v5`（v5.9.2）+ kernel。被 `market-data`/`signal-engine`/`order-engine`/`risk-engine`/`backtest-engine` 等通过 `pkg/postgresx` 显式构造客户端消费。禁止反向依赖业务域。
