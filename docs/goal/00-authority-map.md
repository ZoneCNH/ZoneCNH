...
| `module/{module}/v{version}/` | 版本化管线快照    | 自包含：goal/ spec/ design/ plan/ tasks/ prompt/ evidence/（S1-S11 合层） |
| `module/{module}/matrix/`    | 跨版本追溯矩阵    | 模块级追溯记录（模块 SSOT）；跨模块 canonical edge 见 `.config/goal/matrix/` |
| `module/{module}/gate/`      | 跨版本门禁定义    | 模块级门禁检查清单（模块 SSOT）；跨模块 Gate 状态见 `.config/goal/gates/` |
| `module/{module}/schema/`    | 跨版本 Schema     | 模块级数据/API/契约 schema                                  |
