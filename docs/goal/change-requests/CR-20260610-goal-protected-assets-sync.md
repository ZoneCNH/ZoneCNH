...
- `.codex/agents/goal-*.toml` 已补齐并与 [14-agent-protocols.md](../14-agent-protocols.md) 对齐；`.copilot/agents/` 当前缺少 `goal-*` Agent 投影，且 `.copilot/AGENTS.md` 仍将 Copilot 定位为 pipeline leader / scorer。Copilot Goal Agent 同步前，不得宣称这些 Copilot Goal Agent 已实现。
...
- 缺失 Copilot Goal Agent 投影会让执行者假设不存在的 agent 能力，从而破坏协作和审计预期。
...
3. 检查 `.claude/agents/`、`.codex/agents/` 与 `.copilot/agents/`，确保 agent prompt 包含单任务单 writer、worktree 隔离、多源 reviewer、pipeline-arbiter、Gate 不绕过、Evidence Bundle 和 Change Request 边界；若 Copilot `goal-*` agent 缺失，应在审批后补齐或继续作为投影缺口记录。
...
2026-06-11: 已执行第 1、3、4 项中的保守同步：`.config/goal/schema/rules.yaml` 显式投影 G10 `validation_summary`；`CONSTITUTION.md` 对齐主流程与 Matrix 横切定位；`.claude/agents/goal-*` 中的旧文档数量、Release Gate 和 Evidence 字段口径已同步；`.codex/agents/goal-*.toml` 已补齐 Goal 专用投影。`.copilot/agents/goal-*` 投影仍缺失；由于仓库未提供已批准的 Copilot Goal Agent 模板和运行时契约，本轮不凭空生成，继续作为后续 P2 投影缺口处理。
