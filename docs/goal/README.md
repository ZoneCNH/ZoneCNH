...
| 模块 Feature Spec、Traceability、Task、Prompt 输入与模块级 Goal 文档 | `module/` | 作为编码前模块制品事实源，通过 Goal ID / Task ID / 路径被引用；模块级 Goal 固定为 `module/{module}/goal.md`，不得使用 `goal/` 目录或 `goal/1.md`；实现代码统一位于 `/home/{module}` 对应仓库 |
