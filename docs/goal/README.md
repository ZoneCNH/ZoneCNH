...
| 模块 Feature Spec、Traceability、Task、Prompt 输入与模块级 Goal 文档 | `module/` | 作为编码前模块制品事实源；模块采用目录化结构（管线 goal/spec/design/plan/tasks/prompt/test/review/release/retrospective + 横切 schema/matrix/gate/evidence/registry）；Goal 文档位于 `module/{module}/goal/goal.md`；Code 在 `/home/{module}` |
...
模块采用管线+横切双层结构：管线 `goal/spec/design/plan/tasks/prompt/test/review/release/retrospective`（S1-S11，Code 在 `/home/{module}`）+ 横切 `schema/matrix/gate/evidence/registry`。Goal 文档位于 `module/{module}/goal/goal.md`。`.config/goal/` 为跨模块控制面 SSOT；模块级横切目录为本地投影。
