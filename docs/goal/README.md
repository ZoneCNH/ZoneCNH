...
| 模块 Feature Spec、Traceability、Task、Prompt 输入与模块级 Goal 文档 | `module/`                                                                       | 作为编码前模块制品事实源，通过 Goal ID / Task ID / 路径被引用；模块采用目录化结构（goal/spec/design/plan/tasks/prompt/schema/matrix/gate/evidence/registry），Goal 文档位于 `module/{module}/goal/goal.md`；实现代码统一位于 `/home/{module}` 对应仓库 |
...
模块采用目录化管线结构：`module/{module}/goal/`、`spec/`、`design/`、`plan/`、`tasks/`、`prompt/`、`schema/`、`matrix/`、`gate/`、`evidence/`、`registry/`。Goal 文档位于 `module/{module}/goal/goal.md`。`.config/goal/` 为跨模块控制面 SSOT（Registry、Matrix canonical edge、Gate 状态、Evidence Bundle、Pipeline 状态），模块级 `schema/`、`matrix/`、`gate/`、`evidence/`、`registry/` 为本地投影。
