...
| 数据域 | 行情、宏观、另类数据采集                                                                                                      | market_data (14: 1 dispatch + 12 SDK + 1 C/S Module), macro_data (11), alternative_data                                                                                       |
| 分析域 | 因子计算、特征存储、因子评估、市场状态识别、宏观体制识别、数据流管线、M×S 联合决策；活跃模块采用 market_regime / macro_regime / regime_engine / ms_brain / flowx，market_engine / macro_engine 仅保留在投影与迁移文档 | factor_engine, feature_store, factor_eval, market_regime, macro_regime, regime_engine, ms_brain, flowx                                                              |
