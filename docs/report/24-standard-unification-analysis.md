...
> 本报告是 Goal 体系统一修复前的历史审查快照。当前规则入口以 `docs/goal/` 与 `.config/goal/` 为准；报告中的 legacy 目录结构描述仅保留为历史问题背景。
...
| 维度 | 统一度 | 判断 |
| ---- | ------ | ---- |
| 运行时目录 | 82 | `.config/goal/` 已成为主要运行时目录，但旧模板当时仍保留分散的 legacy 文档制品目录结构 |
...
- `09-templates.md` 当时仍保留 goal/spec/matrix/task/plan/prompt 分散目录结构。
