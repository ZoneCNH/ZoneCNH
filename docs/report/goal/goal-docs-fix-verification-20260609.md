...
剩余扣分项是 P2/P3 级工程化增强，不阻塞当前 90+ 目标：专用负例 fixture 套件尚未单独沉淀，且当前 worktree 存在与本轮无关的历史脏文件/未跟踪目录。
...
| `.config/goal/schema/rules.yaml` ignore 检查 | 被 `.gitignore:12:!.config/goal/schema/**` 放行 |
...
- 当前 worktree 有与本轮无关的未跟踪目录和历史删除项，未在本轮清理。
