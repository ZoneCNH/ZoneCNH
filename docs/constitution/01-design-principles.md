...
| P3   | `resiliencx` 只做运行时弹性    | timeout/retry/circuit/bulkhead/rate/fallback 属于它，交易风控属于 `riskx`（原 `risk_engine`，2026-06-22 已重命名）                             |
...
| 编号 | 原则                           | 含义                                                                                             |
| ---- | ------------------------------ | ------------------------------------------------------------------------------------------------ |
| P5   | 风控是独立引擎                 | 策略只能通过 `riskx` 提交订单，不能直接调用 `orderx`（原 `risk_engine` / `order_engine`，2026-06-22 已重命名） |
| P6   | 回测与实盘共享代码             | `signal_factory` / `factor_engine` / `riskx` 同一套，`backtestx` 只替换数据源和撮合/回放环境 |
...
| P10  | 执行抽象交易所差异             | `orderx` 对上层暴露统一接口，内部适配各交易所（原 `order_engine`，2026-06-22 已重命名）                                                |
