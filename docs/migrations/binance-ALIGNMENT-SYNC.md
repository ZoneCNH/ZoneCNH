...
>
> **仓库迁移记录**：binance 运行时仓已随 ZoneCNH→xhyperium 组织迁移，当前路由为 `github.com/xhyperium/binance`（registry.yaml `repo`/`owner` 同步更新为 `xhyperium`）。历史提交/引用中可见的旧组织路径属历史快照，不影响当前路由。
...
- **零遗漏**：全树（含隐藏、排除 `.git`）对旧仓库 URL 模式扫描返回 **NONE**；无新增或残留旧组织路径引用。
