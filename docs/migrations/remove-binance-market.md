...
| 日期 | 事件 |
|---|---|
| 2026-06-05 | `ZoneCNH/binance-market` 仓库创建 |
...
`binance-market` 仓库已于 2026-06-17 通过 GitHub 仓库删除（`gh api -X DELETE repos/ZoneCNH/binance-market` 等价操作）彻底移除：

- 仓库 URL `https://github.com/ZoneCNH/binance-market` 返回 404
- `gh api repos/ZoneCNH/binance-market` 返回 `Not Found`
...
> 旧 `binance` SDK 仓库（`github.com/ZoneCNH/binance`）保留，作为 `module/binance` C/S Module 的 monorepo 运行时仓库继续承载新架构。两者重名无关：旧 SDK 仓库已被新模块完全接管。
