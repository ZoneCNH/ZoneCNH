...
> **Rubric auto-scorer**：[`scoring/rubric-score.py`](scoring/rubric-score.py) 提供六阶段（spec/matrix/tasks/plan/prompt/code）的机器评分，作为 rules 源的 Spec→Code 阶段实现。Design/Test/Review/Release/Retrospective 阶段的 auto-scorer 待扩展。
...
- **受保护文件清单**（见宪法 §14.1）：`docs/governance/scoring/RUBRIC-*.md`、本文件、`ARBITER-PROTOCOL.md`、`.claude/agents/`、`.codex/agents/`、`.copilot/agents/`、`.omc/state/outer-metrics/`、`.omx/state/outer-metrics/`、`.copilot/state/outer-metrics/`、`CONSTITUTION.md`。所有 LLM agent **只读**。
...
普通阶段产物的 RSI 是自动修复；评分体系、rubric、agent、arbiter、工作流入口等受保护文件的 RSI 是元级改进流程。后者必须先创建 `docs/governance/improvements/{YYYYMMDD}-{slug}/SPEC.md` 并通过同一条管线，再执行宪法 §14.3 的 fork、A/B、outer metric 与人类批准。
