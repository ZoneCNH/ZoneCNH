...
> **管线定位声明**（2026-07-03 P2.1 更新）：Goal→Retro（G0-G11）是唯一管线。本文件定义的 Spec→Code（S1-S6）是 G2-G6 的**快速通道子集**，适用于已有明确需求的模块。当本文件与 `docs/goal/` 冲突时，以 Goal 管线为准。Gate 判定以 `docs/goal/04-gates.md` G0-G11 为权威裁决；本文件定义的四源评分（claude/codex/copilot/rules）为 Goal Gate G2/G5/G6/G9 的 score 实现机制。详见 `docs/goal/00-authority-map.md` §管线优先级。
...
| 文档                                         | 用途                                                    |
| -------------------------------------------- | ------------------------------------------------------- |
| `docs/workflow/README.md`                    | **工作流统一入口** — 统一管线导航、阶段总览             |
