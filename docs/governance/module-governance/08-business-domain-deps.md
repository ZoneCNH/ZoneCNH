...
> 本专题规划将 FOUNDATION-DEPS.yaml 依赖矩阵从基座层扩展到业务域层，闭合"业务域无机器可读依赖矩阵"缺口。本专题为规划文档，实施需各业务域仓库协调。
...
| 阶段 | 范围 | 产物 |
| --- | --- | --- |
| Phase A | L2.5 补登记（4 模块） | FOUNDATION-DEPS modules 段扩展 |
| Phase B | 执行域登记（riskx/orderx/positionx/settlement） | business_domain_modules + allowed/forbidden |
| Phase C | 决策域登记（signal_factory/backtestx/optimizer/strategyx/maestro） | 同上 |
| Phase D | 分析域登记（factor_engine/feature_store/...） | 同上 |
| Phase E | 数据域登记（market_data/macro_data/...） | 同上 |
| Phase F | CI 集成（xlibgate 扩展 import graph 检查覆盖业务域） | xlibgate 更新 |
...
本专题定义 schema 与推广路径，**不实施** FOUNDATION-DEPS.yaml 修改。实施由各业务域 owner 协调，作为后续工作。每个阶段实施时须更新本专题状态。
