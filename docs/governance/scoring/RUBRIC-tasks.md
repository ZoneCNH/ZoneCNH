...
| spec_ref 闭合    | 15     | 每个 Task 的 FR/BR/AC 覆盖可从 task 文件定位验证（通过 spec_ref 字段、acceptance_criteria 或 Acceptance 章节中的 FR/BR/AC ID 引用） |
...
- 任一 Task 无法追溯到 Spec — 即 task 文件中既无 spec_ref 字段，也不在 acceptance_criteria、Acceptance 章节或其他可定位位置引用 FR/BR/AC ID。
- 任一 Task 跨模块。
