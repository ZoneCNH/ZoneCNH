...
> 评分对象：`specs/{module}/tasks/TASK-*.md` 全集
> 总分：100。
...
| 维度 | 满分 | 检查重点 |
|------|------|----------|
| Task 模板符合度 | 12 | 遵循 `specs/TASK-TEMPLATE.md` 结构，章节齐全 |
...
`composite_score = min(三平台评分)` 且 `composite_score >= 98`、无红线、无低置信度、分差在阈值内 → 进入 Plan 阶段。
