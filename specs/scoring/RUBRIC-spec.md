...
机器评分 `composite_score = min(三平台评分)` 且 `composite_score >= 98`、无红线、无低置信度、分差在阈值内。`Status: Approved` 由 arbiter pass 后自动翻转；`spec-review` 仅作为评分证据和对抗性参考，不构成独立 gate。
