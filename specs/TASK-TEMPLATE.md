...
| `module` | ✅ | 所属模块名，与 `specs/{module}/SPEC.md` 对应 |
| `scope` | ✅ | 一句话描述本 task 做什么，不超过 100 字 |
| `spec_ref` | ✅ | 至少一条引用，格式为 `specs/{module}/SPEC.md#{FR/BR/AC-ID}` |
...
TASK-REDISX-003:
  ...
  spec_ref:
    - "specs/redisx/SPEC.md#FR-009"
    - "specs/redisx/SPEC.md#FR-010"
    - "specs/redisx/SPEC.md#BR-005"
  files:
    - "locker.go"
    - "locker_test.go"
    - "redlock.go"
...
TASK-CONFIGX-001:
  ...
  spec_ref:
    - "specs/configx/SPEC.md#FR-001"
    - "specs/configx/SPEC.md#BR-001"
  files:
    - "loader.go"
    - "loader_test.go"
  acceptance_criteria:
...
```markdown
请根据 specs/{module}/SPEC.md 拆分 Task Spec。
...
| 文档 | 关系 |
|------|------|
| `specs/*/SPEC.md` | Task Spec 的来源，每个 task 必须引用 spec 中的 FR/BR/AC |
| `docs/ai/prompt-templates.md` Section 2 | 任务拆分 prompt 的详细版本，本模板定义结构，prompt 定义流程 |
| `specs/TRACEABILITY.md` | Task 完成后更新 Status 列 |
| `specs/DEFINITION-OF-DONE.md` | Task 完成的验收条件 |
