...
> - [x] 独立 reviewer 签字（见 `REVIEW-VERDICT.md`，Verdict: `APPROVED_FOR_STRUCTURE`）
>
...
| FR-007 | 定义 15 条基本真理（TRUTH-001\~015） | P0 |
...
本地 hooks + CI gate + GitHub Ruleset，三者必须同时生效（FR-047 5 层执行链）。本地文件不能证明 GitHub Ruleset 已启用——见 §23.7.3 远端治理不可本地证明项。
...
| NG-24  | branch protection 未启用 / required checks 不匹配 | `goalcli remote-attest branch-protection` | `pack/branch-protection.json`（远端，§23.7.3） |
...
### 23.7 待解决问题（Open Questions）

#### 23.7.1 Open Questions
...
#### 23.7.4 参考资料：文档清单（Documentation Inventory）

> 本子节是 §23.7 内的参考性映射表，不引入额外硬约束。

##### 23.7.4.1 标准文档（docs/standard/，27 个）
...
#### 23.7.5 参考资料：关键数字与映射（Key Figures & Mappings）
...
|------|------|
| 输入文件总数（当前整理口径） | 154 |
| 主规格工件总行数（不含 archive） | 动态统计；提交前以 `wc -l specs/xlib-standard/{SPEC,CONFLICT-LEDGER,COVERAGE-MANIFEST,TRACEABILITY,README}.md` 为准，不写入静态汇总。 |
| 规则总数 | 419 |
| P0 active | 119 (100%) |
| P1 active | 244 |
| 合计 active | 363 (87%) |
...
##### 23.7.5.4 15 条基本真理（TRUTH-001~015）
