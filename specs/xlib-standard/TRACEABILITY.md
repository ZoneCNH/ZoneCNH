...
> 本表当前 FR 来源覆盖 **100%**（52/52，pinned 2026-06-08 05:25 +08:00，commit `93753b30`）。证据类型按 `line` / `file` / `validator-output` 分层；非 `line` 证据不得计入行级覆盖。

| 维度 | 当前状态 | 缺口 | 收敛计划 / 时限 |
|------|----------|------|------------------|
| 条款级追溯（章节 → 来源） | ✅ 完整 | — | 维持 |
| FR 来源覆盖（FR-NNN → 来源文件 + 证据类型） | ✅ 100% (52/52) | — | 维持 |
...
> 共 52 条 FR。每行列出 FR 编号、所在子节、SPEC.md 行号、主要来源文件、源行号、证据类型、优先级。
...
| FR | 子节 | SPEC.md 行号 | 名称 | 主要来源 | 源行号 | 证据类型 | 优先级 |
|----|------|------:|------|----------|--------|:--------:|:----:|
...
| `FR-007` | §7.1 | L246 | 定义 15 条基本真理（TRUTH-001~015） | `.worktree/v3.0.md` | 95, 98 | line | P0 |
...
> 来源：AC 编号取自 SPEC.md §23.1（AC-T01\~T04 / AC-I01\~I04 / AC-G01\~G03 / AC-R01\~R06），TC 编号取自 SPEC.md §17.5（TC-001\~TC-017）。
...
| FR-006 | 采纳状态机入口约束 | AC-I01 | TC-013 |
| FR-007 | 15 条基本真理 | AC-T02 | — |
| FR-008 | 9 个正式 ADR | AC-T03 | — |
| FR-009 | 公共 API 模板 | AC-T02 | TC-001 |
| FR-010 | 9 种 ErrorKind | AC-T02 | TC-014 |
| FR-011 | 9 个最小 metrics | AC-T02 | TC-015 |
| FR-012 | HealthCheck JSON schema | AC-T02 | TC-005, TC-010 |
| FR-013 | 配置显式传入 | AC-T02 | TC-001, TC-009 |
| FR-014 | 配置 Validate 和 Sanitize | AC-T02 | TC-002, TC-006, TC-007, TC-011, TC-017 |
...
| FR-027 | Release Manifest | AC-R01 | TC-012 |
...
| FR-050 | 采纳状态机（8 状态） | AC-I01 | TC-013 |
| FR-051 | 6 个禁止状态转换 | AC-I01 | TC-013 |
...
> **缺口说明**：FR→TC 覆盖率 19%（10/52）仅反映 SPEC.md §17.5 显式列出的 17 个 TC。
> 其余 42 个 FR 的 TC 由 harness.yaml 66 个 gate 输出 + Evidence pack 间接证明（见 §17.2）。
> 下游模块 SPEC §17 应沿用 TC-001\~TC-017 模板并扩展覆盖。
