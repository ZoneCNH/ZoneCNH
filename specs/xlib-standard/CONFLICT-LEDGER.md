...
冲突：当前整理口径使用 154 个输入文件；`DEEP-ANALYSIS.md` 仍保留 181 个源文件的历史口径。
...
取舍：当前口径统一为 66 个 Gates 总数，其中 17 个为 Required（11 个核心 Required + 2 个 guard + 3 个 registry + 1 个 kernel downstream），49 个为 extended/governance/release 等非 Required 分类。
当前发布验收以 `SPEC.md` 中的 gate 定义、release-final 和 preflight 要求为准。
