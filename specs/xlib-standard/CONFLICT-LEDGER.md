...
冲突：历史材料曾同时使用 154 个输入文件和 181 个源文件口径；旧版 `SPEC.md` 与 `DEEP-ANALYSIS.md` 保留过 181 个源文件的历史口径。

取舍：本次整理以 154 个输入文件为当前覆盖口径，当前 `SPEC.md`、`TRACEABILITY.md`、`COVERAGE-MANIFEST.md` 和本账本均使用 154。181 口径只作为 `DEEP-ANALYSIS.md` 等旧分析背景，不得覆盖当前主规格、追溯表或覆盖清单。

## 17. Required Gates 17+ vs harness gates 66

冲突：历史材料把 17+ Required Gates 与 66 gates 混用，容易被读成互相矛盾的当前验收标准。

取舍：17 只表示核心 Required gate 家族；当前 `SPEC.md` 统一为 66 个 Gates（17 Required + 49 扩展/治理/发布）。当前发布验收以 `SPEC.md` 中的 gate 定义、release-final 和 preflight 要求为准。
