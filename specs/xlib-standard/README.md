...
- `SPEC.md`：**当前可执行主规格**（23 节、52 FR、104 WHEN/THEN），其结构、FR 连续性、WHEN 子句和追溯完整性必须由 spec-lint + traceability-check 校验。
...
- `MODULE-SPEC.md` 保留为历史整理工件；其独有内容（来源层级、生成器规格、goalcli 运行时、L2 Provider、Evidence 协议、远端治理、DONE 模板）已合并入 SPEC.md。
...
- 修改输入范围后，必须同时更新 `SPEC.md`、`TRACEABILITY.md`、`CONFLICT-LEDGER.md` 和 `COVERAGE-MANIFEST.md`；历史/参考工件仅在免责声明或历史口径需要同步时更新。
