#!/usr/bin/env bash
# 10-round completeness verification for Plan 008 issues (v3 — worktree-safe + all-state GH query)
set -uo pipefail
...
export BEADS_DIR="${BEADS_DIR:-/home/ZoneCNH/.beads}"

SSOT_COUNT=$(jq '.tasks | length' "$JSON")

# GitHub live: all issues with plan008 label. Closed issues must remain part of the alignment set.
GH_LIVE=$(gh issue list --repo "$REPO" --state all --label plan008 --limit 200 --json number,title --jq '.[] | "\(.number)\t\(.title)"' 2>/dev/null)
...
echo "============================================================"
echo "Plan 008 — 10-Round Completeness Verification (v3)"
echo "============================================================"
echo "SSOT tasks:         $SSOT_COUNT"
echo "GitHub live all-state (P008): $GH_LIVE_COUNT"
...
GH_TITLES_JSON=$(gh issue list --repo "$REPO" --state all --label plan008 --limit 200 --json number,title 2>/dev/null)
TITLE_MISMATCH=""
for i in $(seq 0 $((SSOT_COUNT - 1))); do
  TID=$(jq -r ".tasks[$i].id" "$JSON")
